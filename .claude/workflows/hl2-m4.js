...
1. NORTHSIDE'S DETAILS, which every quote prints as its letterhead: trading name, the mark (and a wordmark if it has one), address, phone, email, website, ABN, the GST rate, a quote's default validity in days, the deposit wording, and the terms. The price file already names the business (the catalogue store's business field, "Northside Marine"); what is set here overrides it. src/domain/people/organisation.ts holds the shape. A quote freezes its letterhead AT ISSUE, like its prices: changing the phone number tomorrow never changes a quote given yesterday.
