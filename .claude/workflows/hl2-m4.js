export const meta = {
  name: 'hl2-m4',
  description:
    "Milestone 4: Northside's places with their marks and heroes, its own details, people and appearance, and bringing a new price file in — sweep, build, verify, critique, fix",
  phases: [
...
MILESTONE 4 IS NORTHSIDE ITSELF: its brands with their marks and pictures, its own details and people so that a quote looks like it came from Northside, how the app looks, and bringing a new version of the price file in safely. docs/PLAN.md § "Milestone 4" was the original brief and the owner NARROWED it on 2026-09-23 — everything the plan names for a second business (a second organisation, sub-dealer organisations, a wizard for a business with no price file, invites, a permission matrix) is cut. § "From the original HelmLogic: what HL_2.0 takes, precisely" is still the evidence for what each shape held and what was wrong with it.
...
WHAT IS NOT COPIED FROM THE ORIGINAL, and the reason is in the plan: no self-service superadmin checkbox, no world-writable storage, no "any signed-in user may write anything", no admin-typed passwords. Its roles, permission matrix, sub-dealer organisations and invites are not carried either: Northside is a handful of people at one dealership, and a flag with no consumer is what the original advertised three times.
...
- Every value is a token (docs/CUSTOMISATION.md), and THIS MILESTONE BUILDS THE PANELS THAT SET THEM: /manage is where Northside sets its own mark, colour, pictures and density. Build them: three layers (defaults, Northside's, each person's), one <style> element written at boot, every choice bounded and every consequence measured — a photograph gets a scrim computed from its own pixels, a brand colour is checked against the ground it will sit on, and a choice that cannot be honoured is refused in a sentence with the nearest thing that works. Never silently unreadable.
...
REFERENCES TO DRIVE: Shopify's sales channels and store settings, Stripe's accounts and Connect dashboards, Linear's projects and teams pages, Notion's teamspaces, Slack's workspace settings, Vercel's project overview, Figma's file browser and team pages, GitHub organisation profile pages, Yamaha's and Mercury's own dealer dashboards if public, the Highfield and Stacer brand pages (their own marks at full size), Apple's product family pages, Sonos and Bang & Olufsen's product family navigation, and Squarespace or Wix's site-settings panels.`,
  },
  {
    key: 'manage',
    port: 5321,
    brief: `SCREEN: SETTINGS, at /manage — Northside Marine's own details, its people, and how the app looks. It is the backend a manager configures without a developer, and it is what makes a quote look like it came from Northside.

WHAT IT HOLDS, and nothing more:
1. NORTHSIDE'S DETAILS, which every quote prints as its letterhead: trading name, the mark (and a wordmark if it has one), address, phone, email, website, ABN, the GST rate, a quote's default validity in days, the deposit wording, and the terms. The price file already names the business (the catalogue store's business field, "Northside Marine"); what is set here overrides it. src/domain/people/organisation.ts and src/domain/model/project.ts hold the shape AS IT STANDS, which is ONLY name, industry, createdAt, slug and quoteTerms; setOrganisation and setQuoteTerms write them and no screen calls either. Every other field here is yours to add, in src/domain, with tests. A quote freezes its letterhead AT ISSUE, like its prices: changing the phone number tomorrow never changes a quote given yesterday.
2. NORTHSIDE'S PEOPLE: the few who sell and manage — name, title, phone, email — so a quote says who prepared it and how to reach them, and the door offers the names already known. No permission matrix, no role hierarchy, no invites. If one distinction earns its place — who may change prices and settings — it is ONE switch per person, ENFORCED where those acts are, and a refused act says why in a sentence.
3. HOW THE APP LOOKS — the customisation the owner asked for ("so dynamic so users can control everything and set their own backgrounds"). docs/CUSTOMISATION.md is the specification; read it in full. Northside sets its mark, a brand colour from which the app derives the scale (never eleven values), the door's background, Home's hero, a place's cover, the quote's cover treatment, register density, corner radius and shadow as named steps, a face from a bounded list that ships with the app, and motion. Defaults from tokens.css, Northside's layer, and each person's own layer in prefs, all writing the SAME CSS custom properties through one <style> element written at boot. Every choice is bounded and every consequence MEASURED: a photograph gets a scrim computed from its own pixels, a colour is checked against the ground it will sit on, and a choice that would put text under 4.5:1 is refused in a sentence with the nearest thing that works. Show what it will look like BEFORE it is kept — the letterhead on a live quote preview beside the fields is the best proof a manager can have.
4. ONE DOOR TO THE WHOLE BACKEND: links to rules, levels, places, the shelf and templates (Milestone 5; link by address), so a manager finds every setting from one place.
CUT, because they serve somebody other than Northside: a second organisation, sub-dealer organisations (the sub-dealer PRICE RUNG stays — it is Northside's own level in its own price file), integrations, a modules tab, margin thresholds, invites.

WHAT IS REFUSED, and the reason must be visible: cost or margin on a customer surface, the refusal sentences themselves, the price figure's behaviour, and the contrast floor.

THE HARD QUESTIONS: (1) a settings screen is the easiest place in any app to be boring — what makes the best ones feel like the business rather than a form? (2) what does a colour or picture chooser look like when every choice is measured and some are refused? (3) how does a manager SEE a change land? (4) how are three people drawn so the screen is not an empty table? (5) 390px?
REFERENCES TO DRIVE: Stripe's business settings and branding page (its live receipt preview is the nearest thing to seeing the letterhead as you set it), Shopify's store details and brand settings, Square's and Xero's invoice branding (the logo, colour and terms that print on an invoice), Linear's workspace settings, Figma's variables panel with modes, Apple's Appearance and Wallpaper panes (apple.com), Raycast and Arc's appearance settings, and Canva Brand Kit.
...
THE LETTERHEAD, PRECISELY (docs/research/proposal/quote-spec.md section 4). The mark prints on white paper, so it needs DARK INK; a white-only mark is refused for the quote with that sentence, as Mercury's is, and the imagery round may already have found Northside's own mark with provenance in data/northside/marks-ledger.json. The terms and the deposit wording are Northside's own words, never written for them: an empty field prints nothing. NOTHING IS PRE-FILLED unless docs/DECISIONS.md records the owner's yes to offering the values the original HelmLogic already holds (its proof PDF, HelmLogic tasks/test-evidence/ffr33-sp560-proof/SP560-display-sheet-proof.pdf page 9, prints Northside's street address and phone); if it does, each offered value shows its provenance and a person accepts it. A person's full name, title, phone and email print as Prepared by. FREEZE AT ISSUE with a test: an issued quote carries a snapshot of the letterhead and the person, and changing the ABN afterwards leaves the issued document's text unchanged; src/domain/quote/golden.test.ts is untouched. THE PREVIEW BESIDE THE FIELDS is src/screens/document's own component drawing the most recent real draft, never a fixture; with no draft, a sentence saying so.`,
  },
  {
    key: 'shelf',
    port: 5331,
    brief: `SCREEN: THE SHELF, at /shelf — WHEN A NEW PRICE FILE ARRIVES, and keeping Northside's work safe.

WHY IT IS CALLED THE SHELF and not "import/export": a user never sees the words entity, schema, field type or reference, and "import" is the app's word rather than the dealer's. The engine module is already src/domain/io/shelf.ts. Name the screen for what a dealer does there, and say the direction's own name in your notes.

WHAT IT IS FOR, and nothing more. Northside's suppliers change their prices and its Master Price File is re-issued; today that means somebody re-keying numbers. So:
1. BRING IN A NEW VERSION OF THE PRICE FILE and see exactly what changes before anything is written: the four-way diff (new / changed / unchanged / gone) over 53 tables and 15,691 rows, read the way a dealer reads it — "the SP560 in PVC rises from $41,340 to $42,100 at Cash", "three Yamaha pairings are new", "one trailer is gone" — then "Apply (N changes)", and Undo after. EVERY QUOTE ALREADY ISSUED STAYS EXACTLY AS IT WAS: a quote is a photograph of what was offered on the day.
2. PASTE FROM EXCEL straight into a table (pasteBlock.ts), with the same preview before it writes.
3. BACK UP AND RESTORE Northside's own work — quotes, customers, rules, settings and the pictures it chose — as one file, and restore it into a fresh browser. Until the shared backend of Milestone 6 this is how work moves to a new laptop, so it must be dependable and obvious.
4. The two dangerous acts — clear the sheet (the quotes stay: "A quote is a photograph of what was offered on the day") and forget everything — possible, never accidental, and never behind a modal everybody clicks through.
CUT, because they serve somebody other than Northside: the wizard for a business with no price file, and bringing in a second business's file.

WHAT THE ENGINE GIVES YOU: src/domain/io/envelope.ts (the envelope, with its http/https/data:image/blob allow-list for pictures, measured in envelope.images.test.ts), readEnvelope.ts, apply.ts and memoryApply.ts (**the four-way diff and "Apply (N writes)"** — the screen's centre), evidence.ts, exportPayload.ts, csv.ts + csvSchema.ts + tableCsv.ts, pasteBlock.ts, and restoreAfterClear.test.ts (what survives a wipe). Every applied change goes through the command layer, so it is undoable and says what it did.

THE HARD QUESTIONS: (1) a diff over 15,691 rows — how is it shown so a dealer presses Apply with confidence rather than fear? (2) what does "Apply (2,318 changes)" look like the moment before it fires, and the Undo after? (3) what does a backup say it holds, so a manager knows what they are saving? (4) how is a dangerous act drawn so it is possible but never accidental? (5) 390px?
REFERENCES TO DRIVE: GitHub's pull-request diff and its file-tree summary (the cascade screen already leans on Terraform, so find a different family for your primary), Supabase's migration and diff screens, Xero and QuickBooks' bank-statement import with its match and ignore columns, Shopify's product CSV import with its error report, Figma's library-update panel (the best consumer-facing "here is what will change" in design software), Apple's Migration Assistant and Time Machine restore (apple.com), 1Password's export warnings, and any macOS "are you sure" that is done well.
...
const verify = await agent(
  `You are verifying HL_2.0 as a boat dealer's sales manager would — somebody who reads nothing and expects it to work.\n\n${STATE}\n\nNothing else is running now, so you may run everything. Change only what a command needs to run; report everything else.\n\n1. Full gate: npm test; npm run build; npm run e2e (alone — about 25 minutes idle). Report every number. If a check is red, say whether it is the app or the machine: a 30 s timeout on a walk through the door is the machine, and re-running it alone with --last-failed --timeout 120000 --workers 1 tells you which. Report both readings.\n2. Serve the built app (npx vite preview --port 5361) and DRIVE THE MILESTONE'S EXIT with the browser tools, screenshotting and READING every step: land cold → name → load the file → /manage: fill Northside's details, SET ITS MARK AND A BRAND COLOUR AND SEE THE WHOLE APP CHANGE, add a person → set a colour that would break contrast and see it REFUSED with the nearest thing that works → /places: open a brand, see its mark and hero, choose its cover from the ledger → raise a quote, issue it as that person, and read the letterhead and "prepared by" on the document → change Northside's phone number and confirm the issued quote did NOT change → /shelf: bring in a changed copy of the price file (a copy of data/northside with a few prices edited), read the diff, Apply, Undo, and confirm no issued quote moved → back up, clear everything, restore, and confirm the quotes, customers and settings came back → then walk the whole sale again to be sure nothing broke → after /manage is filled, raise and issue a Stacer 529 Assault Pro (Tournament) quote, print it to PDF (docs/directions/manage/built/529.pdf), open it with the Read tool, and mark docs/research/proposal/quote-spec.md section 13 items 1, 14, 15, 16 and 28 pass or fail with the page.\n3. The same at 390×844 and 1920×1080.\n4. Write docs/directions/built-m4.md: one honest paragraph per screen at each size, screenshots under docs/directions/<screen>/built/. Say plainly where the flow breaks, where it is confusing, where a control does nothing, and where two screens read as the same shape.\n5. REFUSAL ROT: grep every refusal sentence in src/screens/** and src/routes/** and list each with whether it is still TRUE on this tree.\nReport the gate numbers and everything that is wrong.`,
  { label: 'verify', phase: 'Verify' },
)

phase('Critique')
const critic = await agent(
  `You are the independent critic for HL_2.0, read-only except docs/directions/built-critique-m4.md.\n\n${STATE}\n\nJudge the three new screens, and the app they now sit in, as the owner would. Read docs/directions/built-m4.md and look at every screenshot under docs/directions/{places,manage,shelf}/built/ and a sample of the older ones. Read each screen's sweep notes to see what it promised. Then read the source for what a screenshot cannot show. The verify agent's report: ${JSON.stringify(verify).slice(0, 6000)}\n\nFindings, each with a screen and a severity:\n- blocker: an invented place, person, figure or photograph; a picture attached to something it does not depict; a setting that does not reach the screen or the quote it should; a switch that does not enforce; a hard-coded colour, face or picture path; text under 4.5:1 on its real ground; anything under 11px; a control that does nothing; a refusal that is false on this tree; a cost column on a customer surface; a write that bypasses the command layer; a wipe that clears quotes; an appearance choice that ships unreadable rather than refusing; the words entity / schema / field type / reference on a reader-facing surface; any half of the milestone's exit criterion not actually true; a letterhead field that does not reach an issued document; a letterhead field that changes an already issued document; a white-ink mark on paper; a letterhead value nobody typed or accepted; a preview drawn from a fixture.\n- major: anything built for a business other than Northside; two screens that read as one shape; /places and /data reading as one register twice; a layout that fails at 390 or 1920; a mark that is not the showpiece where the owner asked for it; a diff applied with no census before it; a dangerous act reachable by accident or buried behind a modal nobody reads; a screen that lost its sweep's own idea.\n- minor: craft.\nThen answer plainly in wouldHeAccept: would the owner accept these three screens on sight, and if not, name the ONE thing to change first. He has rejected five redesigns; flattery costs him money.`,
