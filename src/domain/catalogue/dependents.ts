/* ============================================================
   ...
   BY NAME (`@/domain/rules/formula` looks the ref up in a map keyed
   ...
   eight currencies —
     ...
     entityPages       the pages and the dashboard doors that go with
                       ...
                       cascade landed in `deleteCascade.ts` and the
                       same count is now the blast radius of an act
                       rather than a warning about a mess
   ...
   PORT NOTE. `features/designer/dependents.ts` whole. In this build
   the CALLERS are the catalogue commands rather than a designer
   screen: `renameField` runs `renameFieldRefs` over the readers it
   finds here, `deleteTable` removes exactly the links `entityDependents`
   names, and each command's counted radius is built out of these
   answers, so the sentence before the act and the act itself cannot
   disagree.
...
import { validateRule, type RuleRunContext } from '@/domain/rules/engine'
 ...
 *
 * Walks the source with the same rules the formula tokenizer lexes
 * by — quoted text is skipped whole (so the words inside
...
/**
 * Everything `deleteTable` takes with the table, worked out before it
 * is asked for.
 *
 ...
 *
 * The "after" schema is built exactly the way `deleteTable` builds it
 * — table gone, its rows gone, every `reference` column aimed at it
 * dropped from every other table — so the blockers reported are the
 * blockers that will exist. The command reads THIS function's answer
 * to decide which columns go, so the two cannot drift apart.
 */
...
/*  A DELETE WAS THE ONLY ACT THAT ASKED. `ruleBreakage` has counted
    ...
    away from a rule. A retype takes away what the column MEANT; a
    re-point aims it at a table where every field a rule hops to is a
    different field. Both are exactly as breaking and neither was
    counted anywhere.
    ...
    a calculated column and a hop through a link whose target has lost
    the field; this reports whichever of those the act would newly
    cause, without naming either. */
 ...
 * table with different columns. `validate.ts` says so in its own
 * words ("reads a field that is no longer on …"), which is why this
...
/*  WHY THIS SECTION EXISTS, AND WHAT IT COST TO LEAVE IT OUT.
    ...
    IN THE OLD STORE `deleteEntity` DID NOT CASCADE INTO EITHER OF
    THEM. It rewrote entities, rows and rules and returned them;
    `views` and `modules` were not in that object. So a page rooted on
    the deleted table kept its record and stopped having a subject,
    and a module kept the dead id in `tableIds` and was quietly one
    table smaller. `deleteCascade.ts` closed that; what this section
    still does is COUNT, before the press, because the only moment
    either loss can still be avoided is before it.

    Pure, like everything above it: the sheet is read by the caller
    and handed in. */
 ...
 * (`catalogue/views/pairs.ts`) binds `left` to the CANDIDATE row and
