...
/* `module_` with its trailing underscore, because `module` is a
   reserved word in this position and the suite's own vocabulary is
   the model's. oxlint's `no-underscore-dangle` is a naming opinion
   about a body that came over verbatim; renaming a ported fixture to
   please a linter is how a settled behaviour quietly changes, so the
   rule is silenced on this line and the name stays. */
...
describe('quotes — three ways in, and the line says which one', () => {
  ...
  it('keeps the order it was handed, which is the diary’s order', () => {
    ...
    expect(search(index, 'zeta hull').quotes.map((h) => h.quote.id)).toEqual(['new', 'old'])
  })
  ...
  it('caps the list', () => {
    const { entities, rowsByEntity } = sheet()
    const index = buildSearchIndex(entities, rowsByEntity, {
      quotes: Array.from({ length: 9 }, (_, i) => quote({ id: `q${i}`, reference: `ZETA-${i}` })),
    })
