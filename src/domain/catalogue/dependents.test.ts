/* ============================================================
   ...
   PORT NOTE. `features/designer/dependents.test.ts`, forty-seven
   cases, none of them edited. The fixtures carry `orgId` because
   every persisted record here does, and the rule fixtures are typed
   through the contract's own `RuleDef` rather than cast; nothing an
   assertion reads has moved.
   ============================================================ */

import { describe, expect, it } from 'vitest'
import type { EntityDef, FieldDef, ModuleDef, RuleDef, ViewBlock, ViewDef } from '@/domain/model'
import {
  entityDependents,
  entityPages,
  fieldViewers,
...
describe('entityDependents', () => {
  ...
  /* the cascade in `deleteTable` drops these silently, and they are
...
describe('entityPages', () => {
  /* the old `deleteEntity` rewrote entities, rows and rules and
     returned them — views and modules were not in that object, so
     everything this function finds SURVIVED the delete holding a dead
     pointer. That is why it is counted before the press and not
     repaired after it. */
  const views = {
...
describe('fieldViewers', () => {
  ...
  const trailers: EntityDef = {
    ...
    fields: [
      field('g1', 'Column 1'),
      field('g2', 'Link', { type: 'reference', refEntityId: 'e1' }),
    ],
  }
