...
import { quoteDoors } from '@/domain/quote/start'
import { handoverFor } from './handover'

/* THE MODULES ARE MINTED, NOT CARRIED ON THE SEED — the old file's
   own words, and the reason it had to boot the store behind an
   IndexedDB mock just to read nine places off it. `mintModules` is
   that same minting, lifted out of the store and pure, so the mock,
   the dynamic imports and `loadNorthsideProject()` all go: the pack
   is read once per worker and the places are minted from it here.
   The nine places, the 25 stock tables and the 28 pairings are the
   same nine, 25 and 28. */
...
const sheet = (): {
  ...
  const { entities, rowsByEntity } = pack.ctx
  const modules: Record<string, ModuleDef> = {}
  for (const m of mintModules(entities, rowsByEntity, { orgId: pack.ctx.orgId })) {
    modules[m.id] = m
  }
