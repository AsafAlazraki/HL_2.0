/* ============================================================
   ...
   `@/domain/catalogue/table/core` and are never reimplemented here.
   ============================================================ */
import { isImageValue, isSystemFieldId, readCell } from '@/domain/model'
import type { CellValue, FieldDef, FieldType, ImageRef, RowData } from '@/domain/model'
...
export const FORMULA_ERROR_TITLES: Record<string, string> = {
  '#ERROR': 'Formula could not be evaluated — review the expression in the sheet view',
...
/* ============================================================
   ...
   in `@/domain/catalogue/views/columns`: the same formatter the view pages,
 ...
 *  print an empty cell. */
export function cellPrintText(field: FieldDef, value: CellValue, text: string): string {
...
export function cellInsetW(opts: {
  ...
  return base + (opts.groupedLead ? GROUP_LEAD_INSET_W : 0) + (opts.pick ? PICK_INSET_W : 0)
}
...
export const markKey = (rowId: string, fieldId: string): string => `${rowId}\u0000${fieldId}`
...
export const byCreatedAt = <T extends { id: string; createdAt: string }>(a: T, b: T): number =>
