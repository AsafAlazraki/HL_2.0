...
const rows: ViewRow[] = [
  ...
  {
    rowId: 'c',
    values: { price: 198610, hp: '450 HP' },
    text: { price: '$198,610', hp: '450 HP' },
  },
  { rowId: 'd', values: { price: null, hp: '' }, text: { price: '', hp: '' } },
  {
    rowId: 'e',
    values: { price: 5000, hp: 'Electric' },
    text: { price: '$5,000', hp: 'Electric' },
  },
]
...
describe('the between filter', () => {
  ...
  it('is open-ended when one bound is left out', () => {
    expect(
      ids(
        applyView(rows, fields, { filters: [{ kind: 'between', fieldId: 'price', min: 20000 }] }),
      ),
    ).toEqual(['b', 'c'])
    expect(
...
describe('an envelope, as two bands on two real columns', () => {
  ...
  const hulls: ViewRow[] = [
    {
      rowId: 'sp560',
      values: { min: '90 HP', max: '150 HP' },
      text: { min: '90 HP', max: '150 HP' },
    },
    { rowId: 'ru230', values: { min: '4 HP', max: '6 HP' }, text: { min: '4 HP', max: '6 HP' } },
    {
      rowId: 'pa600',
      values: { min: '60 HP', max: '115 HP' },
      text: { min: '60 HP', max: '115 HP' },
    },
  ]
