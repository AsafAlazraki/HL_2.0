 ...
 *
 *  PORT NOTE. This interface was declared in `table/tableReadState.ts`,
 *  beside one of the old repo's twenty-nine hand-rolled external-store
 *  subscriptions, which is what served the reader's chosen density.
 *  That store did not come across — a position inside a screen is a URL
 *  search param here, and `tools/check.ts` fails the build on the hook
 *  it was built on — but the SHAPE is a layout fact the pure transform
 *  below computes every `top` from, so it is declared where it is used.
 *  The screen hands its own metrics in. */
...
const flatLayout = (rows: readonly ViewRow[], m: RowMetrics = DEFAULT_METRICS): GridLayout => {
