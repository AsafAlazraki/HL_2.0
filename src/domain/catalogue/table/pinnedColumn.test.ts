...
import { displayFieldOf, type EntityDef, type FieldDef } from '@/domain/model'
import {
  bandsOf,
  buildSections,
  FOLD_MIN_W,
...
/* THE PACK, NOT `buildNorthsideProject()`. The seed is JSON on disk
   now and the fixture reads it once per worker, so the 53 tables
   arrive on a promise, awaited here at the top because the blocks
   below read them while they are being collected. Every width
   asserted below is measured off the same tables. */
const entities = (await loadPack()).entities
...
const everyBand = (e: EntityDef): Set<string> => new Set((e.sections ?? []).map((s) => s.id))
...
describe('the fold used to take it, and does not now', () => {
  it('files the display column inside a band on all 53 tables', () => {
    ...
    const inside = entities.filter((e) => {
      const pin = displayFieldOf(e)
      return pin?.sectionId !== undefined && (e.sections ?? []).some((s) => s.id === pin.sectionId)
    })
