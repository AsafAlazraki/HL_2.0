/* ============================================================
   ...
   PORT NOTE (HL_2.0). Only the bottom half crossed over. The hook —
   `useHeaderRowHeight`, a layout effect reading `getBoundingClientRect`
   off a hidden probe and a `document.fonts.ready` re-measure — is React
   and the DOM, and `src/domain` is neither. It belongs to the sheet
   screen that owns the probe markup, and it will be written there
   against this function: everything ABOVE, including "it only ever
   grows" and "before paint, deliberately", is the specification that
   hook is built to. Nothing about the arithmetic changed.
   ============================================================ */
