/* ============================================================
   ...
   ============================================================ */
import { isSystemFieldId, type ColumnSection, type FieldDef } from '@/domain/model'
import { ADD_COL_W, FIT_MIN_COL_W, FOLD_W, GUTTER_W, MAX_COL_W, widthOf } from './helpers'
 ...
 *  pin the wrong one. */
function pinIndexOf(placed: readonly PlacedSlot[], pinFieldId: string | undefined): number {
...
export function pinWidthOf(layout: ColumnLayout, pinFieldId: string | undefined): number {
...
export type DrawItem =
  { kind: 'slot'; placed: PlacedSlot } | { kind: 'gap'; w: number; key: string }
...
export function fitColumns(
  ...
  const onScreen = fitsWindow ? flex.length : Math.max(0, Math.floor(Math.max(0, room) / each))
...
export function bandsOf(layout: ColumnLayout, pinFieldId?: string): HeaderBand[] {
