...
import { isSystemFieldId, visibleFields, type EntityDef } from '@/domain/model'
...
/* THE PACK, NOT `buildNorthsideProject()`. The seed is a set of JSON
   ...
   arrives on a promise rather than out of a constructor — which is
   the only reason `entities` is filled in `beforeAll` rather than at
   the top of the file. Every figure asserted below is the same
   figure, read off the same 53 tables. */
let entities: EntityDef[]

beforeAll(async () => {
  entities = (await loadPack()).entities
})
...
describe('what the rows are called', () => {
  ...
  it('gives a relationship the dealer’s word — its rows are pairings', () => {
    ...
    const joins = entities.filter((e) => e.role === 'join')
  ...
  it('gives the KIND its own word, for a count that spans several tables', () => {
    ...
    const boats = entities.filter((e) => e.kind === 'boat')
