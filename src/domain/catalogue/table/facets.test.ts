...
/* THE PACK, NOT `buildNorthsideProject()`. The seed is JSON on disk
   now and the fixture reads it once per worker, so the world arrives
   on a promise — which is the only reason `seed` is filled in
   `beforeAll`. It carries the same `entities` and `rowsByEntity` the
   constructor returned, so every read below is unchanged. */
let seed: PackFixture

beforeAll(async () => {
  seed = await loadPack()
})

/** The tables by id. `ctx.entities` IS that map — the fixture builds
 *  it — so this is a read rather than a second copy. */
const entities = (): Record<string, EntityDef> => seed.ctx.entities
    ...
    for (const entity of seed.entities) {
      ...
      for (const facet of readFacets(entity, rows)) {
        ...
        const inside = applyView(rows, entity.fields, {
          filters: [{ kind: 'between', fieldId: facet.fieldId, min: facet.lo, max: facet.hi }],
        })
