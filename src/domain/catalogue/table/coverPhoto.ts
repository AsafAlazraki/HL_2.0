...
import { isImageValue, primaryImage, type EntityDef, type RowData } from '@/domain/model'

/* ------------------------------------------------------------
   THE HELD COPY IS INJECTED, NOT READ.

   `seededCopy` was a browser-side registry in `lib/imageSources`,
   filled at start-up from the files under `public/seed-images`;
   reading it from here would make this module a store reader, and
   `src/domain` has no store. The QUESTION it answered is unchanged
   and is still the whole point of the file — "do we ship the bytes
   for this address, and at what size" — so it arrives as a
   parameter. The pack's image ledger (`ImageLedgerEntry.file`,
   `.width`, `.height`) is what answers it now, and a caller that
   holds the ledger builds one of these from it.

   A caller that has no ledger gets `NOTHING_HELD`, which is an
   honest empty rather than a guess: nothing is held, so no table
   gets a cover and every card draws its own crest. That is exactly
   the refusal the header argues for.
   ------------------------------------------------------------ */

/** Does the repository hold a copy of this address, and how big is it?
 *  `null` for every address we do not ship. */
export type HeldCopy = (src: string) => { at: string; w: number; h: number } | null

/** No ledger was handed over, so nothing is held. */
export const NOTHING_HELD: HeldCopy = () => null
...
export function coverPhoto(
  ...
  heldCopy: HeldCopy = NOTHING_HELD,
): CoverPhoto | null {
