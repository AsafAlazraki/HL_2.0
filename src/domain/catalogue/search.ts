 ...
 *  the one kind that lives outside the catalogue entirely
 *  (`@/state/quotes.ts` keeps them behind the quote repository), and
...
const linkFieldsOf = (entity: EntityDef, entities: Record<string, EntityDef>): FieldDef[] =>
  entity.fields.filter(
    (f) =>
      f.type === 'reference' && f.refEntityId !== undefined && Boolean(entities[f.refEntityId]),
  )
  ...
  for (const entity of all) {
    ...
    const subject =
      isPairList(entity, links) && !isRetired(entity)
        ? links.find((f) => f.refEntityId !== undefined && !pairIds.has(f.refEntityId))?.refEntityId
        : undefined
    tables.push({
      entityId: entity.id,
...
export const normalizeQuery = (raw: string): string => raw.trim().replace(/\s+/g, ' ').toLowerCase()
