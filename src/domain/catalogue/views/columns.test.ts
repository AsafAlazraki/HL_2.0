/* ============================================================
   ...
   src/domain/model already document ("Beam", "OA Length m",
   "Min HP"/"Max HP"); the rest are obviously synthetic.
   ============================================================ */
import { describe, expect, it } from 'vitest'
import type { EntityDef, FieldDef, FieldType, ImageRef } from '@/domain/model'
import {
  defaultColumns,
  formatCell,
  formatNumber,
...
describe('rangePairs', () => {
  ...
  it('never reads a word that merely starts with min/max as a bound', () => {
    expect(
      rangePairs(
        table([field('f1', 'Minor Column', 'number'), field('f2', 'Maximal Column', 'number')]),
      ),
    ).toEqual([])
