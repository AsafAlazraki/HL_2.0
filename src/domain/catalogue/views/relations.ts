...
import { isRetired, type EntityDef, type ViewBlock } from '@/domain/model'
...
export function findJoinTable(
  ...
  for (const e of Object.values(entities)) {
    ...
    const source = e.fields.find((f) => f.type === 'reference' && f.refEntityId === sourceEntityId)
    const target = e.fields.find((f) => f.type === 'reference' && f.refEntityId === targetEntityId)
