...
function ask(
  ...
  /* the blocks a page opens with, which is exactly what
     `createViewFor` put on the view it made — the registry it made it
     in is gone, so the derivation is called directly */
  return bestAnsweredRow({
    entities: s.entities,
    rowsByEntity: s.rowsByEntity,
    entity: s.entities[BOAT],
    rows: s.rowsByEntity[BOAT],
    blocks: defaultBlocksFor(s.entities, BOAT),
