...
import { DISCONTINUED_FIELD_ID, type EntityDef, type FieldDef, type RowData } from '@/domain/model'
...
const TRAILERS = (extra: Partial<EntityDef> = {}): EntityDef =>
  table('trailers', 'NSM Custom Trailers', [field('tname', 'Model', 'text')], extra)
...
const pairRows = (): RowData[] =>
  trailerRows().map((t, i) => row(`p-${i}`, 'join', { jboat: 'boat-1', jtrailer: t.id }))
...
describe('AN EXISTING QUOTE STILL RESOLVES', () => {
  ...
  const oldQuote: QuoteDef = {
    ...
    /* THE CHAPTERS AND THE AUDIT, WHICH THE CONTRACT NOW REQUIRES.
       A quote frozen before either existed is the case this test is
       about — a document written against stock that has since been
       discontinued — so the honest value for both is the empty one:
       this fixture's walk is its single section, and nothing has
       happened to it since it was issued. The assertion below is
       untouched; `quoteTotals` reads neither. */
    chapters: [],
    lines: [frozenLine],
    adjustments: [],
    events: [],
...
describe('the joins a page never drew', () => {
  ...
  it('draws the relation, and withholds nothing, while both are live', () => {
    ...
    expect(existingRelations(entities, 'boats')).toEqual([{ otherId: 'trailers', joinId: 'join' }])
