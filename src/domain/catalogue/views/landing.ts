...
import { isDiscontinued, type EntityDef, type RowData, type ViewBlock } from '@/domain/model'
import { joinRefFor, makeEngine, relatedRows, type Ctx } from './pairs'

/* ============================================================
   THE PAGE ARRIVES AS ITS BLOCKS, NOT AS AN ID TO LOOK UP.

   This asked `getViewDef(viewId)` — the view-def shadow registry,
   a module-level `Map` published through React's own
   external-store hook because the store had no action for a
   `ViewDef` yet. HL_2.0 drops
   that registry (a view is a persisted table now, `ViewDef.orgId`
   says so in the contract), and a pure module may not hold a
   registry of anything anyway.

   So the blocks are handed in. The rule below is unchanged to the
   letter: it still counts, per row, how many of the PAGE'S OWN
   blocks have an answer, and a caller that has the page passes
   `view.blocks`. What goes is one indirection and one hidden read;
   what stays is every word of the measurement above.
   ============================================================ */
...
export interface BestAnsweredArgs {
  ...
  /** the page's own blocks, in the order it asks them */
...
export function bestAnsweredRow(args: BestAnsweredArgs): BestAnswered | undefined {
  const { entities, rowsByEntity, entity, rows, blocks } = args
