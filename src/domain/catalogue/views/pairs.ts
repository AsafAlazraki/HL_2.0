...
import { compareValues } from '@/domain/rules/engine'
import { createEngine } from '@/domain/rules/engine/evaluate'
import type { NodeSite, RowRef, RuleEngine } from '@/domain/rules/engine/evaluate'
 ...
 *  It used to be declared here, because the contract had no name for
 *  it; the contract carries all three pair fields now
 *  (`@/domain/model`, `rows.ts`), so this is a re-export and no
 *  longer a second declaration of the same literal. */
export { PAIR_ORDER_FIELD } from '@/domain/model'

const SITE: NodeSite = { label: 'This view' }

/* WHAT THIS FILE READS OF THE WORLD — the shape that became the
   contract's own. `CatalogueCtx` (`@/domain/model/ctx.ts`) says in
   its header that this `Ctx` "had already drawn the shape a pure
   module actually needs (entities and rows)" and widened it to every
   field a ported module reads. So the two are one type now: the slice
   is named here for the readers that only ever had two maps, and a
   whole `CatalogueCtx` satisfies it without conversion. */
export type Ctx = Pick<CatalogueCtx, 'entities' | 'rowsByEntity'>

/* ============================================================
   THE WRITES, HANDED IN — they were `useProjectStore.getState()`.

   Four functions below write: `ensureJoinTable` mints a join table,
   and `writePair` and `clearRecommended` add or amend a pair row.
   Each used to reach the store for the action AND for the rows it
   needed to read first. `src/domain` is pure, so the reads come off
   the `Ctx` above and the acts are callbacks the caller passes in —
   the same three store actions, with their signatures unchanged, so
   a caller wires them by name and nothing here learned a new idea
   about what a write is.

   IT IS STILL ONE UNDO STEP. The collapsing is the caller's, exactly
   as it was: `record()` wrapped the burst around these calls before
   and wraps it around them now. Nothing in this file has ever known
   about undo.
   ============================================================ */
...
export function ensureJoinTable(
  ...
  const existing = findJoinTable(ctx.entities, sourceEntityId, targetEntityId)
  if (existing) return existing
  const a = ctx.entities[sourceEntityId]
  const b = ctx.entities[targetEntityId]
  if (!a || !b) return null
  ...
  const made = writes.createJoinEntity(sourceEntityId, targetEntityId, joinTableName(a, b))
  if (!made) return null
  return {
...
/* ---------------------------------------------------------- */
/* Writing pairs — every write goes through an injected action */
/* ---------------------------------------------------------- */
...
function findPairRow(
  ...
  const rows = ctx.rowsByEntity[join.entityId] ?? []
...
function labelPair(
  ...
  const srcEntity = ctx.entities[sourceEntityId]
  const src = (ctx.rowsByEntity[sourceEntityId] ?? []).find((r) => r.id === sourceRowId)
  const joinEntity = ctx.entities[join.entityId]
  const targetRef = joinEntity?.fields.find((f) => f.id === join.targetFieldId)
  const tgtEntity = targetRef?.refEntityId ? ctx.entities[targetRef.refEntityId] : undefined
  const tgt = targetRef?.refEntityId
    ? (ctx.rowsByEntity[targetRef.refEntityId] ?? []).find((r) => r.id === targetRowId)
    : undefined
  const left = srcEntity && src ? rowLabel(srcEntity, src) : ''
  const right = tgtEntity && tgt ? rowLabel(tgtEntity, tgt) : ''
  if (left === '' && right === '') return
  writes.updateCell(join.entityId, joinRowId, join.labelFieldId, `${left} · ${right}`)
}
...
export function writePair(
  ...
  for (const sourceRowId of at.sourceRowIds) {
    ...
    let row = findPairRow(ctx, join, sourceRowId, targetRowId)
    if (!row) {
      const made = writes.addRow(join.entityId, {
        [join.sourceFieldId]: sourceRowId,
        [join.targetFieldId]: targetRowId,
      })
      if (!made) continue
      row = made
      labelPair(ctx, writes, join, at.sourceEntityId, row.id, sourceRowId, targetRowId)
    }
    if (patch.origin !== undefined) {
      writes.updateCell(join.entityId, row.id, PAIR_ORIGIN_FIELD, patch.origin)
    }
    if (patch.recommended !== undefined) {
      writes.updateCell(join.entityId, row.id, PAIR_RECOMMENDED_FIELD, patch.recommended)
    }
    if (patch.order !== undefined) {
      writes.updateCell(join.entityId, row.id, PAIR_ORDER_FIELD, patch.order)
    }
  }
}
...
export function clearRecommended(
  ...
  for (const sourceRowId of sourceRowIds) {
    ...
    for (const pair of readPairs(ctx, join, sourceEntity, sourceRow).values()) {
      ...
      seen.add(pair.rowId)
      writes.updateCell(join.entityId, pair.rowId, PAIR_RECOMMENDED_FIELD, false)
    }
  }
}
