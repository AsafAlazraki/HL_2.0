...
/* DEEP IMPORT, DELIBERATELY, and the direction is the safe one:
   `quote/pricing` reads `@/domain/model`, `@/domain/money` and its own
...
/* ============================================================
   ...
   `@/domain/quote/pricing` already settles both halves of this and
...
export function bandOf(entity: EntityDef | undefined, field: FieldDef | undefined): string {
 ...
 *  money — one format, `@/domain/money`, the same one the quote prints. */
...
export function formatRange(min: CellValue, max: CellValue, name = '', band = ''): string {
