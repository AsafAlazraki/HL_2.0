...
export function betweenRule(
  ...
  return {
    combinator: 'AND',
    clauses: [compare(targetFieldId, 'gte', minFieldId), compare(targetFieldId, 'lte', maxFieldId)],
  }
}
...
export function pickedRule(targetFieldId: string, op: CompareOp, rootFieldId: string): ClauseGroup {
...
function linkSuggestion(root: EntityDef, target: EntityDef): RuleSuggestion | null {
  const inbound = target.fields.find((f) => f.type === 'reference' && f.refEntityId === root.id)
  if (inbound) {
  ...
  const outbound = root.fields.find((f) => f.type === 'reference' && f.refEntityId === target.id)
  if (outbound) {
 ...
 */
const isBookkeeping = (key: string): boolean => BOOKKEEPING.has(key) || /\b(code|id)$/.test(key)
