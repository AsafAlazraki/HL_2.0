/* ============================================================
   ...
   PORT NOTE. This is `features/designer/columnFacts.ts` whole; the
   one thing that moved is the import path of the contract. It lives
   beside the catalogue commands because `retypeField` is its caller
   now — the plan it makes is the blast radius the command says and
   the cells the command writes, so the sentence a person agrees to is
   the sentence that comes true.
 ...
 *
 * `addField` in the old store fell back to `Field N`, which is the
 * one word this whole surface exists not to say — the header comment
 * on the old designer promised table and column, never entity, schema
 * or field, and then the first thing the Add button handed back was a
