/* ============================================================
   ...
   PORT NOTE. `features/designer/columnFacts.test.ts`, twenty-six
   cases, none of them edited. The fixture rows carry `orgId` because
   every persisted record here does; nothing else moved.
   ============================================================ */

import { describe, expect, it } from 'vitest'
import type { FieldDef, RowData } from '@/domain/model'
import {
  cellText,
  columnFacts,
  convertCell,
...
describe('cellText', () => {
  ...
  it('counts pictures rather than printing an address', () => {
    ...
    expect(
      cellText([
        { id: 'i1', src: 'https://example.invalid/a.jpg' },
        { id: 'i2', src: 'https://example.invalid/b.jpg' },
      ]),
    ).toBe('2 pictures')
  })
})
...
describe('convertCell', () => {
  ...
  it('carries no picture anywhere', () => {
    expect(
      convertCell([{ id: 'i1', src: 'https://example.invalid/a.jpg' }], 'text'),
    ).toBeUndefined()
  })
})
