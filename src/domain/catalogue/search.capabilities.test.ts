...
import type { EntityDef, ModuleAccess, ModuleDef, RowData } from '@/domain/model'
import { loadPack } from '@/test/fixtures/pack'
import { buildSearchIndex, optionsOf, search, withinReach } from './search'

/* The pack in place of the seed — read once per worker, at the top,
   because the measurements below are taken inside a `describe` body
   and the world has to be there when it runs. */
...
const answers = (
  ...
  optionsOf(
    search(buildSearchIndex(entities, rowsByEntity, { modules, moduleDoor: true, roleId }), 'zeta'),
  )
    .map((o) => o.id)
    .sort()
...
describe('a hidden table never turns a pair list into a destination', () => {
  it('drops the pair list rather than letting it open itself', () => {
    ...
    const rowsByEntity = {
      ...
      tPair: [
        row('tPair', 'p1', {
          'tPair.name': 'Zeta Closed Row · Omega Motor',
          'tPair.boat': 'rShut',
        }),
      ],
    }
...
describe('the prepared file, measured', () => {
  /* THE PACK IN PLACE OF THE SEED. `buildNorthsideProject()` is gone;
     the fixture reads `data/northside/` once per worker and hands back
     the same tables and the same rows, already keyed by id. Nothing
     below is measured differently. */
  const entities = pack.ctx.entities
  const rowsByEntity = pack.ctx.rowsByEntity
  ...
  it('takes a closed brand file out of every kind at full scale', () => {
    ...
    const before = buildSearchIndex(entities, rowsByEntity, { modules })
    const after = buildSearchIndex(entities, rowsByEntity, {
      modules: closed,
      roleId: 'r-sales',
    })
    ...
    expect(after.tables.some((t) => hidden.has(t.entityId) || hidden.has(t.destId))).toBe(false)
