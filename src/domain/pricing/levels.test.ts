...
const entity: EntityDef = {
  id: 'boats',
  /* `orgId` is required on every persisted record now */
  orgId: 'org-a',
  name: 'Highfield',
  accent: 'blue',
  kind: 'boat',
...
/* THE PACK, NOT `buildNorthsideProject()`. The seed is JSON on disk
   now and the fixture reads it once per worker, so the file arrives
   on a promise — awaited here at the top, because the blocks below
   read it while they are being collected. The 588 variants, the 8
   Series and the 66 Models are the same rows, so every figure below
   is the figure the old suite counted. */
...
describe('the column that made the majority rule necessary', () => {
  ...
  it('and marks none of its 199 as differing, because there is nothing to differ from', () => {
    const sport = hf.root.children.find((c) => c.label === 'Sport')
    const { rows: standing } = standingsAt(
      hf,
      sport?.key ?? '',
      fieldNamed(highfield.entity, 'Warranty'),
    )
