 ...
 * `@/domain/catalogue/views/columns`; a caller that wants it passes its own
 * `textOf` rather than this file growing a second one.
 */
export function valueText(value: CellValue, field: FieldDef, refLabel?: RefLabel): string {
...
function readerOf(model: LevelModel, opts: BuildOpts): (row: RowData, field: FieldDef) => string {
...
const count = (n: number, noun: LeafNoun): string => `${n} ${n === 1 ? noun.one : noun.many}`
 ...
 * prevent.
 */
function noAnswerWhy(tally: Tally, label: string, field: FieldDef, noun: LeafNoun): string {
