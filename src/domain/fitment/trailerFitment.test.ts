...
/* `?raw` is Vite's own specifier and `trailerFitment.raw.d.ts` declares
   its default export for TypeScript. oxlint's import plugin resolves the
   specifier with the query stripped, reads the real module, and reports
   the default it does not have — a false positive about a module that
   exists only at build time. The guard this import serves (F10: the
   selector may never name a length column) is asserted below by reading
   this source text, so the import stays and the rule is silenced here
   rather than turned off for the repository. */
// oxlint-disable-next-line import/default
import selectorSource from '@/domain/fitment/trailerFitment.ts?raw'
...
import { WORKBOOK_RULES, WORKBOOK_RULES_REFUTED } from '@/domain/rules/constraints/workbookRules'
...
/* THE PACK IS WHERE THE SEED LIVES NOW. `buildNorthsideProject()` was a
   ...
   disk once per worker. The two shapes a fitment run needs — the tables
   and their rows — are the same shapes, so every measurement below is
   the same measurement over the same 53 tables and 15,691 rows. */
const pack = await loadPack()
const seed = { entities: pack.entities, rowsByEntity: pack.rowsByEntity }
...
describe('the ATM floor is a floor', () => {
  ...
  it('reads each band its own weight column and never Max Load', () => {
    ...
    for (const table of boatTables) {
      /* vitest's `expect(actual, message)` takes the second argument and
         prints it on failure — which is the only reason a reader would
         know WHICH of the seven bands lost its weight column. oxlint's
         `vitest/valid-expect` is modelled on jest's one-argument expect
         and accepts a literal message but not one read off the table, so
         it is silenced on this line rather than the assertion rewritten. */
