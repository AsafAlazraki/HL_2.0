...
import { buildConcepts, fieldOn, representativeFieldId } from '@/domain/rules/constraints/columns'
import { isDiscontinued, readCell } from '@/domain/model'
...
/* The real sheet, off the pack — see the note in `reading.test.ts`. */
...
function ruleFor(severity: 'block' | 'warn'): ConstraintDef {
  return {
    id: `test:${severity}`,
    /* the tenant key the contract now requires on every record: this
       rule is written against the pack's own sheet, and the pack is
       Northside's */
    orgId: 'northside',
    kind: 'implies',
    if: {
