/* ============================================================
   ...
   WHY THIS FILE EXISTS. `src/domain/rules/configure` is an arc-consistency
...
import { isDiscontinued, isRetired, readCell, rowLabel } from '@/domain/model'
import type {
  AccentKey,
  CellValue,
  Clause,
...
import {
  ...
  type ColumnConcept,
} from '@/domain/rules/constraints/columns'
...
export interface RigCatalogue {
   ...
   *  is the failure `@/domain/catalogue/curation` exists to end. */
 ...
 */
function observedValues(concept: ColumnConcept, project: RigProject): CellValue[] | undefined {
...
export function readRig(input: RigInput): RigReading {
  ...
  slots.sort((a, b) => Number(b.narrowed) - Number(a.narrowed) || a.name.localeCompare(b.name))
 ...
 */
export function readStarters(project: RigProject, reading: FanReading, limit = 4000): RigStarter[] {
