/* ============================================================
   ...
   the canvas edits and `@/domain/rules/engine` runs, keeping the node ids
...
import { OUT_HANDLE, TABLE_KINDS, displayFieldOf, isImageValue } from '@/domain/model'
import type {
  CellValue,
  Clause,
  ClauseGroup,
...
import { domainFor, type ColumnConcept, type ValueDomain } from '@/domain/rules/constraints/columns'
import {
  INDICATIVE,
  opsFor,
  valueWords,
  type SentenceCtx,
} from '@/domain/rules/constraints/describe'
...
export type FitRight =
  { k: 'source'; fieldId: string } | { k: 'word'; value: CellValue } | { k: 'none' }
...
export function compileFit(draft: FitDraft, base?: RuleDef): FitCompiled {
  const idFor = (kind: RuleNodeKind): string =>
    (base
      ? nodeOf(base, kind === 'start' ? 'start' : kind === 'match' ? 'match' : 'output')?.id
      : undefined) ?? newId()
