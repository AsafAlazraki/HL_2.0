...
import { isDiscontinued, isRetired } from '@/domain/model'
import { readFanOut, readRoles, subjectKindOf, type FanProject } from './reading'

/* THE SEED ARRIVES FROM THE PACK NOW. The old suite called
   `buildNorthsideProject()` and indexed its tables by id; the pack is
   the same 53 tables and 15,691 rows read off `data/northside/`, keyed
   by the seed's own keys. Every figure below is the one this file
   already asserted — none of them was re-measured to fit. */
const seed = await loadPack()
const project: FanProject = {
  entities: seed.ctx.entities,
...
describe('the arithmetic reconciles with the sheet', () => {
  ...
  it('never folds a carried fact into the pairing count', () => {
    ...
    for (const fan of reading.fans) {
      const table = fan.groups.filter((g) => g.via === 'table').reduce((n, g) => n + g.pairs, 0)
      const column = fan.groups.filter((g) => g.via === 'column').reduce((n, g) => n + g.pairs, 0)
