...
import { PAIR_ORIGIN_FIELD, isDiscontinued, isRetired, readCell } from '@/domain/model'
import type { AccentKey, EntityDef, FieldDef, RowData, TableKind } from '@/domain/model'
 ...
 *
 *  `PairOrigin` in @/domain/model is the app's own vocabulary — 'rule'
...
export function readFanOut(project: FanProject, kind?: TableKind): FanReading {
  ...
  for (const subject of subjectTables) {
    ...
    for (const { group, reached } of byRole.values()) {
      group.strands.sort(
        (a, b) => b.pairs - a.pairs || a.partnerTableName.localeCompare(b.partnerTableName),
      )
