/* ============================================================
   ...
   Almost none of this is new, and that is the point.
     ...
     · THE GRAMMAR is `@/domain/catalogue/table/core`'s one parser, two
...
import { UID_FIELD_ID, displayFieldOf } from '@/domain/model'
import type { CellValue, EntityDef, FieldDef, FieldType, RowData } from '@/domain/model'
import { cellToText, coerceCellText, parseDelimited } from '@/domain/catalogue/table/core'
import {
  describeColumn,
  inferColumnType,
  measureColumn,
  ...
  return cols.map((col) => {
    ...
    if (p.choice.to === 'field') {
      if (taken.has(p.choice.fieldId)) {
        ...
        if (alt)
          return { choice: { to: 'skip' } as MapTo, how: 'nothing' as const, newRefusal: alt }
        const { type, options } = inferColumnType(col.stats)
        return {
...
export function planPaste(input: PastePlanInput): PastePlan {
  ...
  const missing = entity.fields
    .filter(
      (f) =>
        f.id !== UID_FIELD_ID && f.type !== 'formula' && f.type !== 'image' && !written.has(f.id),
    )
    .map((f) => f.name)
...
export function previewPaste(input: PreviewInput): PreviewRow[] {
  ...
  for (const r of rows) {
    const key = cellToText(r.values[display.id] ?? null, display)
      .trim()
      .toLowerCase()
...
export interface PasteWriter extends TableWriter {
  addField: (entityId: string, partial: Partial<Omit<FieldDef, 'id'>>) => FieldDef | null
}
