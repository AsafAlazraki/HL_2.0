/* THE BYTE-ORDER MARK IS IN THE NOTE THAT EXPLAINS IT. Detail 3
   below is that a mark left on the front of the text becomes part of
   the first header cell, and it shows the reader what that looks
   like by printing one. Removing it to please the linter would take
   the evidence out of the argument. */
/* oxlint-disable no-irregular-whitespace */
/* ============================================================
   io/csv — the four things a FILE needs that a clipboard does not.

   The codec itself is `@/domain/catalogue/table/core`: one parser, two
...
import { parseDelimited, serializeDelimited } from '@/domain/catalogue/table/core'
