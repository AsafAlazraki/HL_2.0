...
/* ============================================================
   THE DOORS THIS MODULE KNOCKS ON.

   Everything below used to reach the running app directly:
   `useProjectStore.getState()` for the sheet and its eight mutators,
   `registerViewDef`, `registerConstraints`, `getQuote` and
   `registerQuote` for the three registries. `src/domain` is pure, so
   they arrive as one object of callbacks and every body below is
   unchanged around them.

   `sheet()` IS A FUNCTION AND NOT A SNAPSHOT, and that is the one
   place this module differs from every other pure module in the
   folder. `CatalogueCtx`'s own contract says a caller takes a
   snapshot at the top of an event and passes it down — right for a
   derivation, wrong for this, because an apply WRITES in the middle
   of itself and then reads back what landed: `restoreDesign` asks
   whether a role is already here after creating the ones before it,
   and re-reads the root table the store gave a page it has just
   minted. The old body called `store()` fresh at each of those
   points for exactly that reason. So the read stays live, the
   adapter answers it from the store, and the reason is written here
   rather than discovered later by whoever wonders why one module
   takes a function.

   THE SEED STAMP IS GONE, and its call with it. The old
   `applyReplace` opened by calling `forgetSeedStamp()`, because "the
   stamp records which build of the prepared set THIS BROWSER was
   seeded with; a replace throws that sheet away, so the record goes
   with it, or the freshness notice would be answering a question
   about a project that is no longer open." This build has no
   freshness machinery to answer for — the pack's version is one
   string in its manifest — so there is no record to forget. The
   argument is kept here because it is the right argument the day
   anything like it is built again.
   ============================================================ */

/** What an apply needs the app to do for it. Each one is the store's
 *  or a registry's own function, with its own signature unchanged. */
export interface ApplyPorts {
  /** the sheet as it stands NOW — see the note above on why live */
  sheet: () => CatalogueCtx
  /** the project's own name and revision, which the catalogue does not carry */
  ...
  updateView: (id: string, patch: { name: string; blocks: ViewBlock[] }) => void
  createModule: (
    tableIds: string[],
    name: string,
    description: string,
  ...
  updateModule: (
    ...
    patch: {
      description: string
      capabilities: ModuleDefPatch['capabilities']
      index: ModuleDefPatch['index']
      accent: ModuleDefPatch['accent']
      order: number
      viewId?: string
    },
  ) => void
  ...
  /** the page feature's own registry, which the store mirrors */
  registerViewDef: (view: ViewDef) => void
  /** the constraint registry's seam, scoped to the current organisation */
  registerConstraints: (list: ConstraintDef[]) => void
  getQuote: (id: string) => QuoteDef | undefined
  registerQuote: (quote: QuoteDef) => void
}

/** The four fields `updateModule` takes off a module, named once so
 *  the port's signature cannot drift from the model's. */
type ModuleDefPatch = Pick<
  NonNullable<ProjectExport['modules']>[number],
  'capabilities' | 'index' | 'accent'
>
...
function restoreQuotes(quotes: QuoteDef[] | undefined, ports: ApplyPorts): number {
  let added = 0
  for (const q of quotes ?? []) {
    if (ports.getQuote(q.id)) continue
    ports.registerQuote(q)
    added += 1
  }
  return added
}

export function applyReplace(data: ProjectFile, ports: ApplyPorts): void {
  /* THE SHEET NOW CAME FROM A FILE, NOT FROM THE SEED. The stamp
     recorded which build of the prepared set THIS BROWSER was seeded
     with; a replace throws that sheet away, so the record went with
     it, or the freshness notice would be answering a question about a
     project that is no longer open. There is no stamp in this build —
     see the header — so there is nothing here to forget. */

  keepingOrganisation(
    ports,
    () => {
      ports.replaceProject({
  ...
  restoreQuotes(data.quotes, ports)
}
  ...
  switch (action.op) {
    ...
    case 'link':
      return {
        ...
        ...(action.values !== undefined ? { values: remapValuesMap(action.values, m) } : {}),
      }
    case 'flag':
  ...
  switch (node.kind) {
          ...
          source:
            node.config.source?.kind === 'linked'
              ...
              : {
                  ...
                  entityId:
                    m.entity(
                      node.config.source?.kind === 'entity'
                        ? node.config.source.entityId
                        : undefined,
                    ) ?? '',
                },
        },
    ...
    case 'action':
      return {
        ...
        kind: 'action',
        position,
        config: { action: remapAction(node.config.action, m) },
      }
...
function remapBlocks(blocks: ViewBlock[] | undefined, m: DesignRefs, fresh: boolean): ViewBlock[] {
  ...
  for (const b of blocks ?? []) {
    ...
    const columns = b.columns?.map((c) => m.field(c)).filter((c): c is string => c !== undefined)
...
function restoreDesign(
  ...
  for (const role of design.roles ?? []) {
    ...
    /* THE DATE TRAVELS WITH THE JOB. `exportPayload` sorts roles,
       ...
       editable field. */
    ports.createRole(role.name, role.description, role.id, role.createdAt)
  }
  ...
  for (const v of design.views ?? []) {
    ...
    ports.updateView(record.id, { name: v.name, blocks })
  ...
  for (const r of restored) {
    ports.updateView(r.id, { name: r.name, blocks: r.blocks })
  ...
  if (constraints.length > 0) {
    ports.registerConstraints(
      fresh ? constraints.map((c) => remapConstraint(c, m, stamp)) : constraints,
    )
  }
}
...
export function applyMerge(data: ProjectFile, ports: ApplyPorts): void {
  const store = ports.sheet()
  const cur = {
    meta: ports.project(),
  ...
  for (const list of Object.values(cur.rowsByEntity)) for (const r of list) currentRowIds.add(r.id)
  ...
  for (const [oldEntityId, list] of Object.entries(rowsIn)) {
    ...
    mergedRowsByEntity[newEntityId] = list.map((r) => {
      ...
      return {
        ...
        /* the row keeps the tenancy it arrived under — the validator
           stamped every record in the file with the business that
           opened it, and a merge moves ids, never owners */
        orgId: r.orgId,
  ...
  restoreDesign({ views: keptViews, modules: keptModules }, keepAsIs, ports)
  /* then the imported design, on the reissued ids */
  restoreDesign(data, m, ports, true)
