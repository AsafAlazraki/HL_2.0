...
/* ONE COPY OF THIS LINE, IN THE FILE THAT OWNS THE KEY. It was
   ...
   IN THIS BUILD THE BUSINESS IS HANDED IN. `currentOrgKey()` read
   the project store to answer "whose sheet is this?"; every record
   now carries `orgId`, the caller holds the snapshot, and so the key
   arrives as an argument rather than being resolved here. There is
   no copy of the line left to drift. */
...
function write(slug: string, rows: Remembered[]): void {
  cache = { slug, rows }
  try {
    shelf().write(key(slug), JSON.stringify(rows))
  } catch {
...
export function remember(
  ...
  const rest = read(orgKey).filter((r) => !(r.tableId === tableId && r.fingerprint === fingerprint))
...
export function recall(
  ...
  const fingerprint = fingerprintOf(headers)
  if (fingerprint === '') return null
  return read(orgKey).find((r) => r.tableId === tableId && r.fingerprint === fingerprint) ?? null
}
...
export function mappingsFor(tableId: string, orgKey: string): Remembered[] {
  return read(orgKey).filter((r) => r.tableId === tableId)
}

export function forget(tableId: string, fingerprint: string, orgKey: string): void {
  write(
    orgKey,
...
/** Test-time reset: the module cache outlives a cleared shelf. */
export function forgetAllMappings(): void {
  cache = null
}

/** Tell me when a mapping is kept or forgotten. */
export function subscribeToMappings(l: () => void): () => void {
  listeners.add(l)
  return () => {
    listeners.delete(l)
  }
 ...
 *  the snapshot a subscriber compares by identity between renders.
 *
 *  The old file's hook filtered inside the snapshot and handed back a
 *  new array each time; `evidence.ts` records what that cost when it
 *  happened there. `read` returns the cached array by reference, so
 *  this is stable between writes and a caller filters against it. */
export const mappingsSnapshot = (orgKey: string): Remembered[] => read(orgKey)
