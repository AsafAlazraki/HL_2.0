...
import { EXPORT_KIND, EXPORT_VERSION, type ProjectExport } from '@/domain/model'

/* The business opening the file. Every record the validator mints is
   stamped with it — it is never read out of the file. */
const ORG = 'northside'
...
const ok = (raw: unknown): ProjectExport => {
  const r = validateEnvelope(raw, ORG)
  if (!r.ok) throw new Error(`expected a valid file, got: ${r.error}`)
  return r.data
}

const table = (data: ProjectExport, id: string) => data.entities.find((e) => e.id === id)
...
describe('the pair columns every curated join carries', () => {
  ...
  it('keeps the well-known ids, which is the only reason a pair can be read', () => {
    ...
    for (const t of ['tJ1', 'tJ2']) {
      expect(table(d, t)?.fields.map((f) => f.id)).toEqual(['__origin', '__recommended', '__order'])
    }
  })
  ...
  it('still refuses two columns sharing one id INSIDE one table', () => {
    ...
    const r = validateEnvelope(file({ entities: [bad] }), ORG)
  ...
  it('still refuses an ordinary column id used on two tables', () => {
    ...
    const r = validateEnvelope(file({ entities: [motors(), twin] }), ORG)
    expect(r.ok).toBe(false)
    if (r.ok) return
      ...
      blocks: [
        {
          id: 'b1',
          tableId: 'tMotors',
          children: [{ id: 'b2', tableId: 'tJoin', children: [{ id: 'b3', tableId: 'tBoats' }] }],
        },
      ],
    })
...
describe('a file saved before any of this existed', () => {
    ...
    entities: [
      {
        ...
        name: 'Boat',
        accent: 'blue',
        fields: [{ id: 'fOld', name: 'Name', type: 'text' }],
        position: { x: 10, y: 20 },
      },
    ],
    groups: [],
  ...
  it('still refuses a file from a version this build cannot read', () => {
    const r = validateEnvelope({ ...v1(), version: EXPORT_VERSION + 1 }, ORG)
    expect(r.ok).toBe(false)
  })
})
...
describe('an imported design is untrusted input', () => {
  ...
  it('rejects a page id that collides with a table id', () => {
    const r = validateEnvelope(full({ views: [view({ id: 'tBoats' })] }), ORG)
    expect(r.ok).toBe(false)
    if (r.ok) return
    expect(r.error).toContain('shares an id')
  ...
  it('rejects two modules sharing one id, which would share one store record', () => {
    const r = validateEnvelope(full({ modules: [moduleDef(), moduleDef({ order: 4 })] }), ORG)
    expect(r.ok).toBe(false)
  })
  ...
  it('says which block is damaged instead of accepting a file shaped wrong', () => {
    for (const key of ['views', 'modules', 'constraints']) {
      const r = validateEnvelope(full({ [key]: 'not a list' }), ORG)
      expect(r.ok).toBe(false)
...
describe('the roles a file’s grants name travel with it', () => {
  ...
  it('refuses a damaged roles block rather than importing half of it', () => {
    const r = validateEnvelope(file({ roles: 'Sales' }), ORG)
    expect(r.ok).toBe(false)
  ...
  it('refuses two roles under one id, because a role id keys a store record', () => {
    ...
    const r = validateEnvelope(file({ roles: [role(), role({ name: 'Workshop' })] }), ORG)
    expect(r.ok).toBe(false)
