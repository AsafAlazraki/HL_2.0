/* ============================================================
   ...
   `./csv` and `@/domain/catalogue/table/core` already parse the delimiter,
...
import type { CellValue, FieldDef, FieldType } from '@/domain/model'
import { coerceCellText } from '@/domain/catalogue/table/core'
...
export function describeColumn(
  ...
  const noun = type === 'number' ? 'a number' : type === 'date' ? 'a date' : 'yes or no'
 ...
 *  bytes, not rows". */
/* The control characters ARE the subject: this refuses a file that is
   bytes rather than rows, and it can only do that by naming them. */
...
function noHeaderRefusal(header: string[]): CsvRefusal | null {
  ...
  const dataish = live.filter(
    (h) => coerceCellText(h, asField('number')).ok || coerceCellText(h, asField('date')).ok,
  )
  ...
  return refusal(
    ...
    `The first row of that file is data, not column names — it reads ${live
      ...
      .join(
        ', ',
      )}${live.length > 3 ? ', …' : ''}. Put a row of column names at the top and upload it again.`,
  )
}
...
function headerRefusals(header: string[]): CsvRefusal[] {
  ...
  const blanks = header.map((h, i) => (h.trim() === '' ? i : -1)).filter((i) => i >= 0)
  if (blanks.length > 0) {
    out.push(
      refusal(
        'blank-header',
        `${blanks.length === 1 ? 'One column has' : `${blanks.length} columns have`} no name in the first row (${blanks
          ...
          .join(
            ', ',
          )}${blanks.length > 4 ? ', …' : ''}). Name every column in row one — this app will not invent a name for a column of somebody's data.`,
      ),
    )
  }
    ...
    return refuse(
      ...
      refusal(
        ...
        `${plural(longRows.length, 'row carries', 'rows carry')} more cells than there are columns (${longRows
          ...
          .join(
            ', ',
          )}${longRows.length > 5 ? ', …' : ''}). That is usually a comma inside a value that is not in quotes. Fix those rows and upload it again.`,
      ),
    )
  }
...
export function describeBuild(columns: number, rows: number, name: string, emptied = 0): string {
