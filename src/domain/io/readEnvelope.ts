...
export type EnvelopeRead =
  { ok: true; data: ProjectFile; fileName: string } | { ok: false; error: string }

/** THE FILE A PERSON PICKED, AS MUCH OF IT AS THIS FUNCTION READS —
 *  a name and a promise of text.
 *
 *  It was typed as the DOM's `File`, and a browser's `File` satisfies
 *  this exactly, so the door in the shell hands one over unchanged.
 *  Written structurally because `src/domain` is pure: naming `File`
 *  here would put a browser type in the middle of the one function
 *  that has to answer identically at two doors, and one of those
 *  doors is a test. Nothing else about a `File` is read — not its
 *  size, not its type, not its bytes. */
...
export async function readEnvelopeFile(file: PickedFile, orgId: string): Promise<EnvelopeRead> {
  ...
  const res = validateEnvelope(raw, orgId)
