...
/* ONE COPY OF THIS LINE, IN THE FILE THAT OWNS THE KEY. It was
   ...
   IN THIS BUILD THE BUSINESS IS HANDED IN. `currentOrgKey()` read the
   project store; every record now carries `orgId` and the caller
   holds the snapshot, so the key arrives as an argument and there is
   no copy of the line left to drift. The paragraph above still says
   which key it must be: the SHEET's, not the signed-in user's. */
 ...
 *
 * The browser's own store throws rather than evicts — which is why
 * `Shelf.write` is declared as a call that may fail — and a log that
 * lost the
...
function write(slug: string, rows: MergeEvidence[]): void {
  ...
  while (attempt.length > 0) {
    try {
      shelf().write(key(slug), JSON.stringify(attempt))
      saved = true
      break
    } catch {
 ...
 *
 *  `at` and `orgKey` ARE HANDED IN RATHER THAN RESOLVED. They were a
 *  `Date.now()` default and a store read; a pure module reads neither
 *  clock nor store, and the caller has both. `who` is the third of
 *  the same kind — it was `currentUser()` here — and it stays
 *  optional because undefined is honest: nobody signed in is nobody,
 *  and "System" is not a name (the same rule `activity.ts` keeps two
 *  folders over). */
...
export function mergesOf(tableId: string | undefined, orgKey: string): MergeEvidence[] {
  const rows = read(orgKey)
  return tableId ? rows.filter((r) => r.tableId === tableId) : rows
}

export function clearMerges(orgKey: string): void {
  write(orgKey, [])
}

/** Test-time reset: the module cache outlives a cleared shelf, which
 *  is the one thing a suite can be caught by. */
export function forgetMerges(): void {
  cache = null
}

/** Tell me when a merge is recorded or the log is emptied. */
export function subscribeToMerges(l: () => void): () => void {
  listeners.add(l)
  return () => {
    listeners.delete(l)
  }
...
/**
 * THE WHOLE LIST FOR ONE BUSINESS, BY REFERENCE AND NOT FILTERED.
 *
 * The question is asked per register — a person standing in Highfield
 * wants to know what landed in Highfield — and the answer is still
 * `mergesOf`. This is the SNAPSHOT a subscriber reads, and it is
 * deliberately the unfiltered list.
 *
 * IT SUBSCRIBES TO THE WHOLE LIST AND FILTERS AFTERWARDS, and that
 * is not a detail. A snapshot subscriber compares what the snapshot
 ...
 * stable between writes; the caller memoises its filter against it.
 *
 * The hook itself is not here: `src/domain` is pure, and the four
 * stores are where a subscription lives now.
 */
export const mergesSnapshot = (orgKey: string): MergeEvidence[] => read(orgKey)
