/* ============================================================
   ...
   in @/features/views/sellable and in @/domain/model, and both are
   exactly the sort of rule that a bulk path quietly forgets.
   ============================================================ */
import { describe, expect, it } from 'vitest'
import { DISCONTINUED_FIELD, DISCONTINUED_FIELD_ID } from '@/domain/model'
...
describe('what it says before it does it', () => {
  it('counts matched, overwritten and new, and names the cell both ways', () => {
    ...
    expect(describePlan(plan)).toBe('3 rows matched, 1 of them is overwritten across 1 cell.')
  })
  ...
  it('names a heading that is not a column here, and adds nothing', () => {
    ...
    expect(plan.refusals.find((r) => r.id === 'unknown-column')?.say).toContain('Dealer margin %')
    expect(plan.changes).toEqual([])
  })
...
describe('an export that comes back untouched', () => {
  ...
  const exact: EntityDef = {
    ...
    fields: [
      F({ id: 'f_name', name: 'Model' }),
      F({ id: 'f_cost', name: 'Landed Hull Cost', type: 'number' }),
    ],
  }
...
describe('two columns wearing one name', () => {
  ...
  const twinRows: RowData[] = [
    {
      ...row('t1', { f_name: 'SUP', f_trade_a: 703, f_trade_b: 0, f_cash: 900 }),
      entityId: twins.id,
    },
  ]
