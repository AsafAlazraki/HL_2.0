...
import { makeCtx, type CatalogueCtx, type QuoteDef, type QuoteLine } from '@/domain/model'
...
import { summariseEnvelope } from './readEnvelope'
import { memoryApply } from './memoryApply'

/* THE SHEET IS EMPTY AND THE QUOTES ARE ON IT — which is exactly
   what the old file set up, by registering two documents in a store
   nothing had seeded. What crosses the file boundary here is the
   DOCUMENTS; the tables are somebody else's suite. The old version
   reached a module-level quote registry through `registerQuote` and
   `allQuotes`; the documents are on the context now and the
   assertions read them from there. */
const ORG = 'northside'

const sheetOf = (quotes: QuoteDef[]): CatalogueCtx => makeCtx({ orgId: ORG, quotes })
...
/* both documents, on one sheet */
const SHEET = sheetOf([DRAFT, ISSUED])

/* ---------------------------------------------------------- */

describe('the envelope carries the quotes', () => {
  it('WRITES THEM AT ALL — the bug this file exists for', () => {
    const file = buildExportPayload(SHEET, PROJECT, true)
  ...
  it('CARRIES NO CUSTOMER NAME IN A STRUCTURE-ONLY COPY — the privacy hole', () => {
    const file = buildExportPayload(SHEET, PROJECT, false)
    ...
    expect(SHEET.quotes).toHaveLength(2)
  })
  ...
  it('accepts its own file and brings both documents back', () => {
    const result = validateEnvelope(buildExportPayload(SHEET, PROJECT, true), ORG)
  ...
  it('counts them on the import preview, so a person sees documents coming', () => {
    expect(summariseEnvelope(buildExportPayload(SHEET, PROJECT, true)).quotes).toBe(2)
  })
})
...
function fileWith(quotes: unknown): unknown {
  const base = buildExportPayload(SHEET, PROJECT, false) as unknown as Record<string, unknown>
  return { ...base, quotes }
}
...
describe('a quotes block from outside is narrowed at the door', () => {
  it('reads a file with no quotes key as a project with no quotes, not as damaged', () => {
    const base = buildExportPayload(SHEET, PROJECT, false) as unknown as Record<string, unknown>
    delete base.quotes
    const result = validateEnvelope(base, ORG)
    expect(result.ok).toBe(true)
    if (!result.ok) return
  ...
  it('refuses a quotes block that is not a list, and says which block', () => {
    const result = validateEnvelope(fileWith({ nope: true }), ORG)
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.error).toBe('That copy is damaged: the customer quotes block cannot be read.')
  })
    ...
    const bad = {
      ...quoteFrom('qt_evil', 'issued', [
        line({
          ...
          label: 'Hull',
          unitPrice: 1000,
          image: { id: 'i', src: 'javascript:alert(1)' },
        }),
      ]),
      subjectImage: { id: 'i2', src: 'data:text/html,<script>' },
    }
    const result = validateEnvelope(fileWith([bad]), ORG)
    expect(result.ok).toBe(true)
    if (!result.ok) return
  ...
  it('treats an unreadable state as GIVEN, never as an editable draft', () => {
    ...
    const result = validateEnvelope(fileWith([odd]), ORG)
    expect(result.ok).toBe(true)
    if (!result.ok) return
  ...
  it('drops a repeated quote id rather than letting one document replace another', () => {
    ...
    const result = validateEnvelope(fileWith([a, b]), ORG)
    expect(result.ok).toBe(true)
    if (!result.ok) return
...
describe('export, replace, import — the documents survive and do not double', () => {
  it('a REPLACE leaves the quotes alone and puts the file’s own back by id', () => {
    const file = buildExportPayload(SHEET, { ...PROJECT, rev: 2 }, true)
    /* the sheet a replace is driven into, holding both documents —
       the store in memory, so the act under test is `restoreQuotes`
       and nothing else */
    const sheet = memoryApply(SHEET, { ...PROJECT, rev: 2 })
    const before = sheet.quotes().length
    ...
    applyReplace(result.data, sheet.ports)
    expect(sheet.quotes()).toHaveLength(before)
    expect(sheet.ports.getQuote('qt_issued')?.state).toBe('issued')
    expect(quoteTotals(sheet.ports.getQuote('qt_draft') as QuoteDef).total).toBe(83480)
  ...
  /* THE CONFIRM SHEET'S OWN SENTENCE is not tested here, and it is
     the one test from this file that did not come across. It read
     `sheetNow()` and `quotesSurviveSentence()` from `io/sheetNow.ts`,
     which counts the live sheet out of the browser's store to write
     the line "Your 2 quotes stay." That module is browser-only and
     the plan drops it; the sentence it wrote is a promise the app
     still keeps — `applyReplace` above is what keeps it, and the test
     above is what pins it. The words themselves belong to whichever
     screen ends up asking the question, and they will be tested
     there, against that screen. */
})
