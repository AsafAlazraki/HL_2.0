...
import { memoryShelf, setShelf } from './shelf'
...
beforeEach(() => {
  forgetMerges()
  /* the shelf, emptied — the browser store this line used to clear is
     an injected seam now (see io/shelf.ts), and in node there never
     was one to clear */
  setShelf(memoryShelf())
})
...
describe('what is worth putting on the record', () => {
  ...
  it('puts the newest first, because that is the one being asked about', () => {
    ...
    expect(mergesOf('e-zeta', ORG).map((m) => m.source)).toEqual(['newer.csv', 'older.csv'])
  })
})
