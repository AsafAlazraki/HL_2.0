/* ============================================================
   ...
   2 · IT NEVER RESURRECTS A DISCONTINUED ROW. `isDiscontinued` is a
       ...
       BY TYPING IN THE CELL, which is what @/domain/model says. A
...
import type { CellValue, EntityDef, FieldDef, RowData } from '@/domain/model'
import { cellToText, coerceCellText } from '@/domain/catalogue/table/core'
 ...
 */
export type MergeIdentity = { on: 'key'; at: number } | { on: 'name'; at: number; field: FieldDef }
  ...
  for (const line of lines) {
    ...
    if (computedFor && checkable.size > 0) {
      ...
      for (const [i, f] of checkable) {
        ...
        const delta = Number.isFinite(theirs) && Number.isFinite(mineNum) ? theirs - mineNum : null
    ...
    for (const r of read) {
      ...
      if (r.field.id === DISCONTINUED_FIELD_ID && isDiscontinued(target) && r.value !== true) {
