/* ============================================================
   ...
   AND THE SHEET IS HANDED IN. It read `useProjectStore.getState()`,
   `getConstraints()` and `allQuotes()` at the top of the call — three
   live reads for one file. `CatalogueCtx` is exactly those three and
   the rest of the world besides, taken once by the caller and passed
   down, so the exporter is pure and a test can hand it the world it
   wants. The one thing the context does not carry is the SHEET'S OWN
   NAME and its revision, which belong to the project rather than to
   the catalogue, so they arrive beside it.
   ============================================================ */

import { EXPORT_KIND, EXPORT_VERSION, type CatalogueCtx } from '@/domain/model'
...
export function buildExportPayload(
  ...
  const constraints = [...s.constraintDefs].sort((a, b) => a.createdAt.localeCompare(b.createdAt))
  ...
  const quotes = includeData
    ? [...s.quotes].sort((a, b) => a.createdAt.localeCompare(b.createdAt))
    : []
  return {
    kind: EXPORT_KIND,
    version: EXPORT_VERSION,
    exportedAt: s.now(),
    project: { name: project.name, rev: project.rev },
