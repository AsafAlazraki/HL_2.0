...
import { bandOf, formatCell, normColumn } from '@/domain/catalogue/views/columns'
...
import { branchNoun, kindNoun, leafNoun } from '@/domain/catalogue/table/grouping'
...
import { existingRelations } from '@/domain/catalogue/views/relations'
 ...
 *  that is gone. */
export function moduleTables(module: ModuleDef, entities: Record<string, EntityDef>): EntityDef[] {
  const out: EntityDef[] = []
...
export function listedTables(module: ModuleDef, entities: Record<string, EntityDef>): EntityDef[] {
...
export const hasPictures = (entity: EntityDef): boolean => imageFieldOf(entity) !== undefined
...
export function priceReadOf(entity: EntityDef): PriceRead | undefined {
  const safe = (f: FieldDef): boolean => !isCostColumn(entity, f) && !inForbiddenBand(entity, f)
  ...
  for (const wanted of FALLBACK_PRICE_COLUMNS) {
    const f = entity.fields.find(
      (x) => (x.type === 'number' || x.type === 'formula') && normColumn(x.name) === wanted,
    )
...
export function groupEntries(entries: IndexEntry[], tables: EntityDef[]): IndexSection[] {
...
/* ============================================================
   WHETHER WE HOLD A COPY OF A PICTURE — injected, not read.

   The old file asked `@/lib/imageSources`: `seededCopy(src) !== null
   || sourceKind(src) === 'own'`. The first is a browser-side registry
   of the copies under `public/seed-images`, and the second reads
   `window.location.origin` — so between them they are a store read
   and a DOM read, and `src/domain` has neither.

   Both halves survive, in the shape this repository gives them:

     · THE HELD COPY IS A PARAMETER. The pack's image ledger
       (`ImageLedgerEntry.file`) is what answers it now, and a caller
       that has the ledger hands it in. A caller that does not gets
       `NOTHING_HELD` — an honest empty: nothing is held, so the
       bigger picture wins and no door draws a blank.
     · OUR OWN PIXELS NEED NO REGISTRY. `sourceKind` returns 'own'
       exactly when the address is `data:image/` or `blob:`, which it
       decides before it ever touches `window` — so that half is the
       same regular expression, written here, and it is DOM-free.
   ============================================================ */

/** Does the repository already hold a copy of this address? */
export type HeldPicture = (src: string) => boolean

/** Our own bytes; nothing is fetched and nothing can fail — the
 *  `own` half of `sourceKind`, verbatim and without the DOM. */
const OWN_PIXELS = /^(?:data:image\/|blob:)/i

/** No ledger was handed over, so nothing is held. */
export const NOTHING_HELD: HeldPicture = () => false

/** The drawers these entries fall into, biggest first inside each
 *  table, tables in the module's own order. */
export function categoryDrawers(
  entries: IndexEntry[],
  tables: EntityDef[],
  heldCopy: HeldPicture = NOTHING_HELD,
): Drawer[] {
  const out: Drawer[] = []
  for (const entity of tables) {
    const word = branchNoun(entity)
    ...
    for (const [name, group] of byBranch) {
      ...
      mine.push({
        ...
        low: low?.price ?? '',
        high: high?.price ?? '',
        img: bestPicture(group, heldCopy),
      })
    }
...
function bestPicture(group: readonly IndexEntry[], heldCopy: HeldPicture): ImageRef | undefined {
  ...
  for (const e of group) {
    ...
    const held = heldCopy(img.src) || OWN_PIXELS.test(img.src)
