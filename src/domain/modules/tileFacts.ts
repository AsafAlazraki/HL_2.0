...
import { bandOf, formatCell, isMoney } from '@/domain/catalogue/views/columns'
...
export function factsFor(
  /* THE TABLE IS IN THE SIGNATURE AND NOT IN THE BODY, and it stays
     there: every caller passes the table it is reading, and a column
     already carries the band it was filed under, so nothing here has
     to look one up. `noUnusedParameters` is on in this repository, so
     the underscore says so out loud rather than the parameter being
     dropped and the call sites rewritten. */
  _entity: EntityDef,
  row: RowData,
