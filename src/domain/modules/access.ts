 ...
 * app had lost it.
 */
export function accessRows(module: ModuleDef, roles: readonly RoleDef[]): AccessRow[] {
  const offered = new Set(module.capabilities)
...
export function withoutRole(module: ModuleDef, roleId: string): ModuleAccess[] | undefined {
...
export const capabilityLabel = (k: ModuleCapability): string => MODULE_CAPABILITIES[k].label
...
export const capabilitySays = (k: ModuleCapability): string => MODULE_CAPABILITIES[k].says
...
export function roleReach(modules: readonly ModuleDef[], roleId: string): number {
  return modules.filter((m) => isUnrestricted(m) || grantedTo(m, roleId).length > 0).length
}
