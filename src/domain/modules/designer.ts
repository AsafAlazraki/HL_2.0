...
import { defaultColumns } from '@/domain/catalogue/views/columns'
import { buildConcepts } from '@/domain/rules/constraints/columns'
...
import { imageFieldOf, priceReadOf, relatedTables, type PriceRead } from './read'
...
export const NOT_YET_SAYS: Partial<Record<DesignerCapability, string>> = {
  ...
  /* IT NAMES WHAT LEAVES, because that is the decision being made
     ...
     the face draws four. */
  export:
    'A file of this register comes off the bar above the Catalog — every column the table holds, cost columns included.',
  import:
    'A file comes back in from the bar above the Catalog, and it says what it would change before it changes it.',
}
...
export function capabilityStates(
  ...
  const ruleable = gone ? [] : buildConcepts(Object.fromEntries(tables.map((e) => [e.id, e])))
 ...
 *
 *  WHY THIS WRITES THE WHOLE DEFINITION. The views feature owns view
 ...
 *
 *  THE SEAM IS NOW A PARAMETER. `registerViewDef` was a store write
 *  read straight out of the module registry; a pure module takes the
 *  write as a callback and the caller hands its own store's version
 *  in. Nothing else about the act moves — the same guard against a
 *  no-op move, the same whole definition, the same stamp — and the
 *  clock comes in the same way so a test can fix it. */
export function moveViewBlock(
  view: ViewDef,
  blockId: string,
  dir: -1 | 1,
  registerViewDef: (next: ViewDef) => void,
  now: () => string = nowIso,
): void {
  ...
  registerViewDef({ ...view, blocks, updatedAt: now() })
}
