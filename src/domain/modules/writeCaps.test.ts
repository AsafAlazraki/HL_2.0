...
import type { EntityDef, FieldDef, ModuleAccess, ModuleCapability, ModuleDef } from '@/domain/model'
import {
  addLabel,
  addSays,
  addedSay,
...
describe('readWrites — the access grid decides it too', () => {
  ...
  it('grants exactly what the grid granted, and withholds exactly the rest', () => {
    const closed = makeModule(WRITES, ['t1'], [{ roleId: 'r-sales', capabilities: ['add'] }])
    const w = readWrites(closed, [t], [t], 'r-sales')
    expect(w.add).toEqual({ on: true })
    expect(w.edit.on).toBe(false)
    expect(w.delete.on).toBe(false)
  ...
  it('a withheld verb carries no blocked sentence — one refusal, not two', () => {
    /* Nowhere to write AND not granted: the reason that applies to
       ...
       problem. */
    const closed = makeModule(WRITES, ['t1'], [{ roleId: 'r-manager', capabilities: ['add'] }])
...
describe('the words — the dealer’s noun, and the table named', () => {
  ...
  it('names the row that was taken out, not "1 row deleted"', () => {
    expect(removedSay('Sport 460', boats)).toBe('Sport 460 was taken out of Highfield Inflatables.')
  })
