...
import { buildConcepts, conceptIndex } from '@/domain/rules/constraints/columns'
import { WORKBOOK_RULES } from '@/domain/rules/constraints/workbookRules'
import {
  constraintsFor,
  flowRulesFor,
  kindOfConceptKey,
...
import { DEFAULT_CAPABILITIES, MODULE_CAPABILITIES } from '@/domain/model'
...
const flowRule = (id: string, rootEntityId: string, searches?: string): RuleDef => {
  const nodes: RuleNode[] = [{ id: `${id}-s`, kind: 'start', position: { x: 0, y: 0 }, config: {} }]
  if (searches) {
    nodes.push({
      id: `${id}-m`,
      kind: 'match',
...
describe('the limits that reach a module', () => {
  ...
  it('changes its answer when the module is pointed elsewhere', () => {
    ...
    const after = constraintsFor(all, { ...BOATS, tableIds: ['mot_yamaha'] }, index).map(
      (c) => c.id,
    )
...
describe('the tenth verb, now that the contract carries it', () => {
  ...
  it('is read off the module, per module, like the other nine', () => {
    ...
    expect(capabilityStates(MOTORS, [HIGHFIELD]).find((s) => s.key === 'configure')?.on).toBe(false)
  })
