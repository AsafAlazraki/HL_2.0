...
import type { ConstraintDef, EntityDef, ModuleDef, RuleDef, TableKind } from '@/domain/model'
import type { ColumnConcept } from '@/domain/rules/constraints/columns'
import { WORKBOOK_RULES, type WorkbookRuleSeed } from '@/domain/rules/constraints/workbookRules'
...
export function flowRulesFor(rules: readonly RuleDef[], module: ModuleDef): GoverningFlowRule[] {
  const mine = new Set(module.tableIds)
  ...
  for (const rule of rules) {
    ...
    const searched = rule.nodes.find((n) => n.kind === 'match' && mine.has(n.config.targetEntityId))
