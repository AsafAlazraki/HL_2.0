...
/* THE COMPONENT HALF DOES NOT CROSS. The old file also exported
   `useBrowsableModules` — `useProjectStore((s) => s.modules)` plus
   `useSessionRoleId()` plus a `useMemo`, memoised on the map and the
   job. That is React and two store reads, and `src/domain` has
   neither; the three lines belong beside the list that draws them,
   over the catalogue store and the session store.

   The identity property survives here, where it is the load-bearing
   one: `browsableModules` hands the SAME object back when nothing is
   hidden, so whatever memo a screen keys on the module map keeps
   bailing out exactly as it did before. `reach.test.ts` asserts it. */
