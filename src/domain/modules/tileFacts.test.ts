...
import { buildEntries, listedTables, priceReadOf } from './read'
import { factColumns, FACTS_PER_TILE } from './tileFacts'
import { isMoney, bandOf } from '@/domain/catalogue/views/columns'
import { mintModules } from './mint'

const pack = await loadPack()

function sheet() {
  return { entities: pack.ctx.entities, rowsByEntity: pack.rowsByEntity }
}
...
describe('the figures a tile carries', () => {
  ...
  it('refuses every money column, including the one that would have won', () => {
    ...
    const { entities, rowsByEntity } = sheet()
    const modules = mintModules(entities, rowsByEntity, { orgId: pack.ctx.orgId })
