/* ============================================================
   ...
   WHAT IS ASSERTED, and why each one is here:
     ...
     1. THE VERDICT, per module, through the same function the mint
        asks when it makes one. Nine modules, and the four that get
   ...
   THE SHEET IS THE PACK, and the places are minted from table keys
   by `mint.ts` — which is where the nine came from in the old app
   too, through the store's own create action. Nothing here builds a
   module by hand: a module written straight into a map would be a
   fixture agreeing with the rule rather than the seed meeting it.
   ============================================================ */
import { describe, expect, it } from 'vitest'
import { loadPack } from '@/test/fixtures/pack'
import { listedTables, moduleFace } from './read'
import { PICTURE_FLOOR } from './face'
...
function facesNow() {
  ...
  return mintModules(entities, rowsByEntity, { orgId: pack.ctx.orgId }).map((m) => ({
    name: m.name,
    stored: m.index,
...
describe('which face a module opens on', () => {
  it('gives the four modules with photographs a catalogue, and the five without a register', () => {
    ...
    /* and the stored field the mint wrote agrees with the
       measurement, because `mintModules` asked this same function */
