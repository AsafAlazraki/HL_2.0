...
/* THE TENANT KEY the contract now requires on every persisted
   record. A fixture names one because a record that does not carry
   it cannot be filed; nothing in this file turns on its value. */
const ORG = 'org-a'

function makeModule(capabilities: ModuleCapability[], access?: ModuleDef['access']): ModuleDef {
  return {
    id: 'm1',
    orgId: ORG,
    name: 'Motors',
...
describe('the module is the vocabulary', () => {
  ...
  it('REFUSES a grant the module does not offer, changing nothing', () => {
    const m = makeModule(['browse', 'search', 'open'], [{ roleId: 'r1', capabilities: ['browse'] }])
  ...
  it('drops a grant that LAPSED when the module lost the capability', () => {
    ...
    const before = makeModule(
      ['browse', 'quote'],
      [{ roleId: 'r1', capabilities: ['browse', 'quote'] }],
    )
...
describe('absent means unrestricted', () => {
  ...
  it('unticking one role’s last box leaves the others in force', () => {
    ...
    expect(withGrant(m, 'r1', 'browse', false)).toEqual([{ roleId: 'r2', capabilities: ['edit'] }])
  })
})
...
describe('writing a grant', () => {
  it('adds the role when it is not on the list yet', () => {
    ...
    expect(withGrant(m, 'r2', 'edit', true)).toEqual([{ roleId: 'r2', capabilities: ['edit'] }])
  })
...
describe('the figures the access screen states', () => {
  ...
  const other = (capabilities: ModuleCapability[], access?: ModuleDef['access']): ModuleDef => ({
