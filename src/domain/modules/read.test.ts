...
/* THE TENANT KEY the contract now requires on every persisted
   record. Nothing in this file turns on its value. */
const ORG = 'org-a'
...
/* THE SUBJECT, AND THE ONE NAME THAT CHANGED IN THE PORT. It was
   `module_` in HL_Playground, spelt with the trailing underscore
   because `module` is a global in a CommonJS file. This repository
   lints `no-underscore-dangle`, and the module the fixture describes
   is the Trailers module, so it is called that. Not one assertion,
   expected value or test name moved with it. */
const trailers: ModuleDef = {
  id: 'm1',
  orgId: ORG,
  name: 'Trailers',
...
describe('what a module lists', () => {
  it('drops a retired table out of the catalogue and keeps it in the module', () => {
    expect(listedTables(trailers, entities).map((e) => e.id)).toEqual(['live'])
    /* the module still NAMES it — the designer strip has to be able
       to show an admin that it does */
    expect(trailers.tableIds).toEqual(['live', 'obsolete'])
  })

  it('never puts a discontinued row on the index', () => {
    const entries = buildEntries(listedTables(trailers, entities), rowsByEntity)
  ...
  it('counts what the page will draw, and states the rest separately', () => {
    const entries = buildEntries(listedTables(trailers, entities), rowsByEntity)
    expect(moduleRowCount(trailers, entities, rowsByEntity)).toBe(entries.length)
    expect(moduleRowCount(trailers, entities, rowsByEntity)).toBe(2)
    /* one discontinued on the live table, one whole retired table */
    expect(moduleHeldCount(trailers, entities, rowsByEntity)).toBe(2)
  })

  it('leaves the sheet exactly as it found it', () => {
    buildEntries(listedTables(trailers, entities), rowsByEntity)
  ...
  it('groups only what it listed, so no heading is left empty', () => {
    const entries = buildEntries(listedTables(trailers, entities), rowsByEntity)
    const sections = groupEntries(entries, listedTables(trailers, entities))
...
describe('what has happened lately', () => {
  it('reports NOTHING for a freshly loaded sheet', () => {
    ...
    const a = moduleActivity(trailers, entities, rowsByEntity, [])
  ...
  it('counts a row only once it has really been changed', () => {
    ...
    const a = moduleActivity(trailers, entities, worked, [])
  ...
  it('names a discontinued row that was edited — the sheet is where it still lives', () => {
    ...
    expect(moduleActivity(trailers, entities, worked, []).edited).toBe(1)
  })
  ...
  it('takes quotes raised on this module and leaves another module’s alone', () => {
    ...
    const a = moduleActivity(trailers, entities, rowsByEntity, [mine, theirs])
  ...
  it('counts a quote raised against a RETIRED member table', () => {
    ...
    const a = moduleActivity(trailers, entities, rowsByEntity, [quote('q1', 'obsolete')])
    expect(a.quoteCount).toBe(1)
  })
  ...
  it('puts the newest first and caps the list, stating the remainder', () => {
    ...
    const qs = days.map((d, i) => quote(`q${i}`, 'live', { createdAt: `${d}T00:00:00.000Z` }))
    const a = moduleActivity(trailers, entities, rowsByEntity, qs)
  ...
  it('leaves the quotes it was handed exactly as it found them', () => {
    ...
    moduleActivity(trailers, entities, rowsByEntity, qs)
...
describe('what a card says about who may work here', () => {
  const restricted = (access: ModuleDef['access']): ModuleDef => ({ ...trailers, access })

  it('says nothing at all about a module nobody has restricted', () => {
    const r = accessReading(trailers)
    expect(trailers.access).toBeUndefined()
  ...
  it('does not count a role left holding a verb the module has switched off', () => {
    /* `trailers` offers browse, search and open. A grant of `edit` made
