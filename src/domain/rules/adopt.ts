/* ============================================================
   ...
   `src/domain/rules/constraints/discover.ts` reads a price file and
   ...
   ANNOTATE AND REMOVE NOTHING (`src/domain/rules/configure/solve.ts`,
   rule 5 in its header). So a kept pattern can be a real rule, in the
   real store, and still be structurally incapable of deleting a row.
   ...
   the `load` out of storage, which matters most of all because
   storage is a text file a person can edit. (In the old app that
   store was the browser's own key-value store, named here in words
   that `tools/check.ts` now reserves for `src/state/prefs.ts`; in
   HL_2.0 the registry is a table on the project database, so the
   seam is the repository read — and it still has to coerce.)
...
import type { Clause, ClauseGroup, ConstraintDef } from '@/domain/model'
 ...
 * a leaf of the domain and must not reach into a feature. The register
 * maps its own record onto this, which also means an old stored record
...
export type Adoption =
  { adopted: ConstraintDef; blocked?: undefined } | { adopted?: undefined; blocked: string }
 ...
 *
 * `orgId` IS NEW, AND IT IS THE CONTRACT'S, NOT THIS MODULE'S: every
 * persisted record in HL_2.0 carries the tenant key, and a rule minted
 * without one could not be filed. It defaults to the honest empty that
 * `makeCtx` uses, so a caller that has named no organisation — a test,
 * or a project before onboarding — mints a rule that says so rather
 * than one that claims to belong to somebody.
 */
export function adoptObserved(
  ...
  if (farField === nearField) {
    return {
      blocked:
        'both sides resolve to the same column, so the rule would compare a value with itself',
    }
  }
