...
import type { EntityDef, FieldDef, FieldType, RowData } from '@/domain/model'
...
describe('blockers — the findings that mean a model cannot work', () => {
  ...
  it('flags two columns of one table sharing a name, as one finding naming both', () => {
    const found = lint([
      table('t1', 'Table A', [field('f1', 'Column 1', 'text'), field('f2', 'COLUMN 1', 'number')]),
...
describe('advisories — the findings that mean a model will not read', () => {
  ...
  it('leaves a brand table alone — its kind already says what one row is', () => {
    ...
    expect(
      ruleIds([table('t1', 'Labour Rates', [field('f1', 'Column 1', 'text')], { kind: 'custom' })]),
