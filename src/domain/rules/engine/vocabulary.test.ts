...
import type { EntityDef, RuleDef, RuleNode } from '@/domain/model'
...
describe('the notes a person reads say "table"', () => {
  ...
  it('a For each with nothing chosen, and one pointed at a table that has gone', () => {
    expect(
      read(rule([node('n-l', 'loop', { source: { kind: 'entity', entityId: '' } })])),
    ).toContain('For each has no table to loop over — choose one.')
    expect(
...
describe('the refusal a RUN prints says "table" too', () => {
  /* `runRule` calls `validateRule` first and a blocker stops it before
     ...
     WHAT IS NOT ASSERTED, AND WHY. `walk.ts` and `effects.ts` carry
     their own copies of three of these sentences (walk.ts:220, :283,
     :409; effects.ts:75, :158, :175) and they were reworded in the
     same pass. They are belt-and-braces — every one of them fires on a
     condition `validateRule` has already raised as a blocker, so
