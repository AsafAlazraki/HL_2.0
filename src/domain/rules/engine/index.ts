/* ============================================================
   Rules engine — public surface (RULES_SPEC.md § "Execution engine").
     ...
     import { runRule, validateRule } from '@/domain/rules/engine'
     import type { RuleRunResult, RuleIssue } from '@/domain/rules/engine'

   Pure TypeScript: no React, no store, no DOM, no Dexie. The only
   dependencies are '@/domain/model', '@/domain/rules/formula' and
   '@/domain/compare'.
...
/* Row-level helpers the canvas and inspector legitimately need: the
   ...
   uses (so a preview never disagrees with a run). The comparison now
   lives in '@/domain/compare', which the solver shares; it is
   re-exported through './evaluate' so this surface is unchanged. */
