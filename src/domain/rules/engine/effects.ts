...
import type { ActionOp, CellValue, RuleNode, ValueExpr } from '@/domain/model'
...
function resolveValues(
  ...
  for (const [fieldId, expr] of Object.entries(values ?? {})) {
    ...
    if (field.type === 'formula') {
      eng.warn(
        site,
        `${site.label}: "${field.name}" is calculated and cannot be written — skipped.`,
      )
      continue
    }
    const ref =
      expr && expr.kind === 'field'
        ? eng.pickOwner(expr.path?.viaFieldId ?? expr.path?.fieldId, scope, other)
        : scope
...
function summarise(entityId: string, values: Record<string, CellValue>, eng: RuleEngine): string {
  const parts: string[] = []
  ...
  switch (action.op) {
    ...
    case 'create': {
      ...
      if (!entity) {
        eng.warn(
          site,
          `${site.label}: creates rows in a table that no longer exists — nothing written.`,
        )
        return undefined
      }
    ...
    case 'link': {
      ...
      if (!join) {
        eng.warn(
          site,
          `${site.label}: links into a link table that no longer exists — nothing written.`,
        )
        return undefined
      }
      if (!match) {
        eng.warn(
          site,
          `${site.label}: reached a row with nothing matched to it, so there is no pair to link.`,
        )
        return undefined
      }
