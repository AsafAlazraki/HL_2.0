...
function execute(rule: RuleDef, ctx: RuleRunContext): RuleRunResult {
  ...
  const stepNode = (node: RuleNode, item: WalkItem, site: NodeSite): Emission[] => {
    ...
    switch (node.kind) {
      ...
      case 'filter': {
        ...
        return eng.evalGroup(node.config?.group, scopes, site)
          ? [carry(item.match, OUT_HANDLE)]
          : []
      }
      ...
      case 'find': {
        ...
        if (!holder || !via) {
          eng.warn(
            site,
            `${site.label}: follows a link field that no longer exists — nothing was found.`,
          )
          return []
        }
        if (via.type !== 'reference' || !via.refEntityId) {
          eng.warn(
            site,
            `${site.label}: "${via.name}" is not a link field, so it cannot be followed.`,
          )
          return []
        }
      ...
      case 'loop': {
        ...
        for (const entry of items) {
          ...
          count += 1
          out.push(
            carry(entry, LOOP_BODY_HANDLE, { ...item.branchTaken, [node.id]: LOOP_BODY_HANDLE }),
          )
        }
        loopCounts.set(node.id, count)
        /* the walk continues once, past the loop, carrying what came in */
        out.push(
          carry(item.match, LOOP_NEXT_HANDLE, {
            ...item.branchTaken,
            [node.id]: LOOP_NEXT_HANDLE,
          }),
        )
        return out
      }
  ...
  const loopItems = (
    ...
    if (via.type !== 'reference' || !via.refEntityId) {
      eng.warn(
        site,
        `${site.label}: "${via.name}" is not a link field, so nothing can be gathered.`,
      )
      return []
    }
    const anchor =
  ...
  const collectOutput = (
    ...
    const columns: ViewColumn[] = (node.config?.columns ?? []).filter((c): c is ViewColumn =>
      Boolean(c?.fieldId),
    )
