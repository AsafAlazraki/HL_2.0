 ...
 * exact five keys `@/domain/model` declares, and `solve` still
...
const becauseOf = (c: ConstraintDef): string =>
  c && typeof c.because === 'string' ? c.because.trim() : ''
...
function prune(
  ...
  if (isWarn(c)) {
    ...
    for (const v of removed) {
      ...
      else if (!list.some((w) => w.constraintId === c.id))
        list.push({ constraintId: c.id, because })
    }
    markWarned(ctx, c.id)
    return false
  }
...
function enforceClause(ctx: Ctx, clause: Clause | undefined, c: ConstraintDef): boolean {
  ...
  if (right.kind === 'field') {
    ...
    let changed = prune(ctx, leftId, (v) => rightValues.some((w) => compare(op, v, w) !== false), c)
...
function sameFieldEqUnion(clauses: Clause[]): { fieldId: string; keys: Set<string> } | undefined {
...
function applyConstraint(ctx: Ctx, c: ConstraintDef): boolean {
  switch (c.kind) {
    ...
    case 'excludes': {
      ...
      /* THE VIOLATION ITSELF, SAID DIRECTLY. Both sides definitely
         hold at once, so the rule is broken by definition — whatever
         its operators. Until the port (2026-09-16) that sentence was
         reachable only as a side effect of enforcing a negation, and
         NEG_OP has no opposite for 'contains', 'startsWith' or
         'endsWith': a rule un-negatable on both sides fell through
         both branches above and said nothing. `contradiction.test.ts`
         pinned it with `it.fails`; this is the fix it asked for.

         It runs AFTER the two branches on purpose. When a side could
         be negated, enforcing it has already raised this same sentence
         (an all-false OR) or emptied a column and named it — and a
         column emptied reads as unknown, so the check below stays
         quiet rather than adding a second sentence for one rule. The
         dedupe key in `addProblem` covers the case where both say the
         same thing. A warning still cannot get here: `addProblem`
         declines it. */
      if (evalGroup(ctx.doms, c.if) === 'T' && evalGroup(ctx.doms, c.then) === 'T') {
        addProblem(ctx, c, `This rule can no longer be satisfied${becauseTail(c)}.`)
      }
      return changed
    }
  ...
  try {
    ...
    if (active.length > 0) {
      ...
      if (rounds >= MAX_ROUNDS) {
        const tail = lastMover
          ? ` — the last one to act was: ${becauseOf(lastMover) || lastMover.id}`
          : ''
        addProblem(
          ctx,
          lastMover,
