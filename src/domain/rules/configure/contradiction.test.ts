/* ============================================================
   ...
   There are exactly five places a `problem` is raised, and all five
   are covered below:
     ...
     applyConstraint, 'excludes'   both sides of an excludes hold at
                    once (added on the port, 2026-09-16 — see below)
   ...
   One case was asserted as a KNOWN DEFECT, with `it.fails`, at
   "a contradiction the solver could not see". The port fixed the
   defect in solve.ts and deleted the `.fails`; the assertion inside
   it is the full-strength one written before the fix, untouched.
   ============================================================ */

import { describe, expect, it } from 'vitest'
import type { CellValue, ClauseGroup, ConstraintDef, FieldDef } from '@/domain/model'
...
describe('a domain emptied is a contradiction, reported', () => {
    ...
    const s = solve({
      ...
      constraints: [
        rule({
          ...
          then: {
            combinator: 'AND',
            clauses: [{ id: 'z1', left: { fieldId: 'kicker' }, op: 'isTrue' }],
          },
        ...
        rule({
          ...
          then: {
            combinator: 'AND',
            clauses: [{ id: 'z2', left: { fieldId: 'kicker' }, op: 'isFalse' }],
          },
    ...
    expect(s.problems).toEqual([
      {
        constraintId: 'b2',
        message:
          'Nothing is left for kicker, because there is no bracket for a kicker on a small hull.',
      },
    ])
  ...
  it('fires from a field-against-field clause, where no literal is involved', () => {
            ...
            clauses: [
              {
                ...
                left: { fieldId: 'a' },
                op: 'gt',
                right: { kind: 'field', path: { fieldId: 'b' } },
              },
            ],
          },
    ...
    expect(s.problems).toEqual([
      {
        constraintId: 'r-gt',
        message: 'Nothing is left for a, because the tender must be longer than the cradle.',
      },
    ])
  })
    ...
    expect(s.problems).toEqual([
      {
        constraintId: 'r-eq90',
        message:
          'Your choice of 300 for motor no longer works, because the hull is not rated for that much power.',
      },
    ])
  })
    ...
    expect(s.problems).toEqual([
      {
        constraintId: 'r-len',
        message:
          'Your choice of 9 for length no longer works, because no trailer in the file is that long.',
      },
    ])
  })
    ...
    const s = solve({
      ...
      constraints: [
        ...
        rule({
          id: 'p1',
          if: and(['hull', 'eq', 'small']),
          then: and(['prop', 'eq', 'alloy']),
          because: 'the alloy prop only',
        }),
      ],
      chosen: { hull: 'small' },
    })
    ...
    const s = solve({
      ...
      constraints: [
        rule({
          id: 'ghost',
          if: and(['gone', 'eq', 'x']),
          then: and(['alsoGone', 'eq', 'y']),
          because: 'a deleted column',
        }),
        rule({
          id: 'real',
          if: and(['hull', 'eq', 'small']),
  ...
  it('two rules that cannot both hold over a column with no enumerable values', () => {
    ...
    const bounds = [
      rule({
        ...
        if: and(['hull', 'eq', 'small']),
        then: and(['length', 'lte', 5]),
        because: 'the small hull is under 5m',
      }),
      rule({
        id: 'r-hi',
        if: and(['hull', 'eq', 'small']),
        then: and(['length', 'gte', 10]),
        because: 'the small hull is over 10m',
      }),
    ]
...
describe('a contradiction the solver could not see', () => {
  /* MEASURED, NOT INFERRED. The three cases below are the same
     ...
     written three ways that differ only in the OPERATOR.
       ...
       contains / contains   -> NOTHING          (the defect, before
                                                  the port)

     WHY. `excludes` only ever acted through `negateGroup`
     ...
     branches of the excludes case did nothing at all. One negatable
     side was enough to save it, because the other branch still ran;
     when both sides were un-negatable there was no branch left.
     ...
     A fix belonged in solve.ts, which this file may not touch. The
     ...
     FIXED ON THE PORT (2026-09-16), in exactly that shape. The
     `it.fails` below became a plain `it` with its assertion as
     written, and the sentinel beside it — the one written to fail
     "the day the fix lands" — now asserts the fix from its end. */
  ...
  it('is seen when NEITHER side can be negated — and nothing is pruned to say so', () => {
    /* Documenting the fix from the other end. Before the port this
       case asserted `problems` EMPTY, so that the day the fix landed
       it would fail and point here; it did, and this is the one
       assertion the fix changed. The problem is raised directly, not
       by pruning: nothing fires, nothing is blocked, both choices
       stand. */
  ...
  it('`excludes` with un-negatable operators on both sides is no longer silent', () => {
    /* Was `it.fails` — the assertion below was the CORRECT one and
       solve.ts did not satisfy it. The `.fails` was deleted on the
       port when `excludes` began raising a problem from
       evalGroup(if) === 'T' && evalGroup(then) === 'T' directly. */
    ...
    expect(s.problems).toEqual([
      {
        constraintId: 'i1',
        message:
          'Your choice of 300 for motor no longer works, because a small hull takes a 9-series motor.',
      },
    ])
  })
