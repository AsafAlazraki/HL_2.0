...
import type { LintFinding } from '@/domain/rules/lint'
import { lintProject } from '@/domain/rules/lint'
...
describe('the rail against Northside Marine', () => {
  /* THE PACK, NOT `buildNorthsideProject()`. Same 53 tables, same
     rows, read off the JSON the browser fetches; the fixture caches
     them per worker, so this block waits on a promise rather than a
     constructor and every figure below is the figure it was. */
  ...
  beforeAll(async () => {
    ...
    entities = project.ctx.entities
    findings = lintProject({ entities, rowsByEntity: project.rowsByEntity })
    ledger = buildLedger(findings)
  })

  /* the figures the panel's header quotes */
  it('is the sheet the header describes', () => {
    expect(Object.keys(entities)).toHaveLength(53)
  ...
  it('states each lesson once per table instead of once per column', () => {
    ...
    const sentences = rolls.reduce((n, r) => n + (r.why === '' ? r.findings.length : 1), 0)
