...
export function asOneOf(group: ClauseGroup | undefined, ctx: SentenceCtx): OneOfShape | null {
...
function clauseTokens(clause: Clause, side: Side, ctx: SentenceCtx, out: SentenceToken[]): void {
...
export function describeConstraint(c: ConstraintDef, ctx: SentenceCtx): string {
...
/* THE CONTEXT IS PASSED IN, AND THAT IS THE ONE CHANGE THIS PORT MADE.
   In the old app `describeConstraint(c)` took one argument and reached
   for `useProjectStore.getState()` through a module-level cache keyed on
   the identity of the store's `entities` and `rowsByEntity` objects — an
   escape hatch `views/viewDefs.ts` used for the same reason, so a list of
   rules naming themselves would not rebuild the column index once per
   rule. `src/domain` is pure here: no store, no React, no DOM. So the
   caller takes ONE `SentenceCtx` at the top of an event and hands it
   down, which is the same saving made explicit, and a test can hand this
   function exactly the world it wants. The cache is gone with the store
   that made it necessary; `SentenceCtx` still memoises its own column
   domains, which is where the row scans actually were. */
...
export const conceptOf = (ctx: SentenceCtx, key: string | undefined): ColumnConcept | undefined =>
  conceptByKey(ctx.concepts, key)
