...
/* The seed is the pack now — the same tables and rows, read off the
   generated JSON once per worker. */
...
describe('one relationship measured alone', () => {
  it('gives the same answer as the whole-file run, candidate for candidate', () => {
    ...
    const readOff = (r: typeof full): string[] =>
      [...r.proposals, ...r.notProposed]
        ...
        .map(
          (c) => `${c.id}|${c.hits}/${c.tested}|${c.discrimination?.meanLeft ?? 'x'}|${c.verdict}`,
        )
        .sort()
  ...
  it('reports no join key, because a join key is not a property of a pair', () => {
    ...
    expect([...full.proposals, ...full.notProposed].some((c) => c.shape === 'join-key')).toBe(true)
  })
...
function table(
  ...
  return {
    id,
    /* the tenant key the contract now requires on every persisted record */
    orgId: 'org-a',
    name,
    ...(kind ? { kind } : {}),
    role,
