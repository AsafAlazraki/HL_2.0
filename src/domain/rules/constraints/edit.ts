...
import type { CellValue, Clause, ClauseGroup, ConstraintDef, ConstraintKind } from '@/domain/model'
import {
  UNSET_FIELD,
  UNSET_VALUE,
  isUnsetValue,
...
export function setClauseOp(
  ...
  if (op === ONE_OF) {
    ...
    if (!clause) return c
    const held = clause.right && clause.right.kind === 'literal' ? clause.right.value : UNSET_VALUE
    ...
    return stamp(
      withSide(c, side, {
        combinator: 'OR',
        clauses: [
          { id: clause.id, left: clause.left, op: 'eq', right: { kind: 'literal', value } },
        ],
      }),
    )
  }
