...
import { readCell, PAIR_ORDER_FIELD, PAIR_ORIGIN_FIELD } from '@/domain/model'
...
/* NOTHING IS MOCKED ANY MORE, AND NOTHING NEEDS TO BE. The old suite
   built the seed by importing `@/demos/northside`, which reached the
   Dexie repository on the way in, so persistence had to be mocked out
   and every import deferred behind `await import` to let the mock hoist
   ahead of it. The pack is JSON on the disk: `loadPack()` reads it once
   per worker and touches no database at all. The subject is unchanged —
   what the seed derives, not what Dexie writes. */
const pack = await loadPack()

const project = { entities: pack.entities, rowsByEntity: pack.rowsByEntity }
...
describe('the motor and rigging relationships, against the real seed', () => {
  ...
  it('offers the same motor against the same hull more than once, and does not dedupe it', () => {
    ...
    for (const j of motorJoins) {
      rows += j.rows.length
      distinct += new Set(j.rows.map((r) => `${text(r, j.boatFieldId)}|${text(r, j.motorFieldId)}`))
        .size
    }
  ...
  it('keeps the derived-vs-typed distinction, and keeps typed as the majority', () => {
    ...
    expect(
      rule,
      "no pairing is marked 'rule' — the importer has stopped reading formulas",
    ).toBeGreaterThan(0)
    expect(added, "no pairing is marked 'added'").toBeGreaterThan(0)
    expect(
      added,
      'typed pairings must outnumber derived ones — 99.4 % of them are typed',
