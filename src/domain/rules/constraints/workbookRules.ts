...
export const WORKBOOK_RULES: WorkbookRuleSeed[] = [
     ...
     ---------------------------------------------------------- */
  {
    ...
    ref: 'F7',
    evidence: 'observed',
    statement: 'A boat set up for a remote helm must not be offered a tiller-handle motor.',
     ...
     ---------------------------------------------------------- */
  {
    ...
    because:
      'the motor library lists the propellers that motor is offered with, and names one as its default',
     ...
     ---------------------------------------------------------- */
  {
    ...
    ref: 'A4',
    evidence: 'asserted',
    statement: "A pairing's Prop Part No. must be the Parts Module code for its Prop Description.",
     ...
     ---------------------------------------------------------- */
  {
    ...
    because:
      'a Helm Master kit belongs with a digitally controlled motor, and this one is mechanical',
     ...
     ---------------------------------------------------------- */
  {
    ...
    ref: 'F8',
    evidence: 'asserted',
    statement: "A boat may only be offered a trailer from a series built for that boat's brand.",
    ...
    blocked:
      'RUNS, BUT NOT AS A SENTENCE. src/domain/fitment/trailerFitment.ts evaluates this ' +
     ...
     ---------------------------------------------------------- */
  {
    ...
    ref: 'F9',
    evidence: 'asserted',
    statement: "A trailer whose rated ATM is below the boat's weight must be flagged, not blocked.",
    ...
    enforcedIn: 'Business rules · The trailer selector',
    blocked:
      'RUNS AS A WARNING, AND MAY NEVER BE MORE. src/domain/fitment/trailerFitment.ts ' +
     ...
     ---------------------------------------------------------- */
  {
    ...
    ref: 'F12',
    evidence: 'observed',
    statement: 'A tiller-steered boat must not be given a two-battery pre-delivery kit.',
    because:
      'a twin-battery install belongs to a remote helm, and a tiller boat has no second circuit to feed',
     ...
     ---------------------------------------------------------- */
  {
    ...
    plainly: 'It adds five columns together. A sentence compares one column to one value.',
    blocked:
     ...
     ---------------------------------------------------------- */
  {
    ...
    kind: 'implies',
    needs: ['boat::source'],
    plainly: 'It tests where a row sat in the spreadsheet, and no column carries that position.',
    blocked:
     ...
     ---------------------------------------------------------- */
  {
    ...
    blocked:
      ...
      'UNTIL BOTH LAND the check runs as a report — src/domain/rules/constraints/registration.ts, ' +
...
export const WORKBOOK_RULES_REFUTED: RefutedRuleRecord[] = [
  ...
  {
    ref: 'F11',
    candidate: "Any rule at all on the trailer's Between Guards or Trailer Length dimensions.",
    measured: 'Between Guards populated on 74 of 476 rows (15.5 %); Trailer Length on 75 (15.8 %)',
    verdict:
...
export function buildWorkbookConstraints(
  ...
  for (const seed of WORKBOOK_RULES) {
    ...
    out.push({
      id: seed.id,
      /* THE TENANT KEY IS THE CONTRACT'S, NOT THIS FILE'S: every
         persisted record in HL_2.0 carries it, and a rule minted
         without one could not be filed. It defaults to the honest
         empty `makeCtx` uses, so a project that has named no
         organisation seeds rules that say so. */
      orgId,
      kind: seed.kind,
      if: sides.if,
...
/** The ledger of seed ids already handed to an organisation.
 *
 ...
 *
 *  IT IS A VALUE NOW, NOT A KEY. In the old app it was a
 *  mirror in the browser's key-value store, keyed
 *  `helmlogic.constraints.seeded.v1`, read at module load and written
 *  behind a debounce — which put a browser global inside the rules. `src/domain` is pure, so the
 *  ledger is handed IN as data and handed BACK as data, and the
 *  organisation's copy of it is persisted through the repository
 *  seam by the caller. Keyed by `orgId` because `readLedger` was
 *  keyed by organisation and the shape must survive the move: two
 *  businesses on one machine were never offered each other's seeds.
 *
 *  The old file's own reason for tolerating a corrupt read is worth
 *  keeping at this end too: the worst case of losing the ledger is
 *  that a seed is OFFERED again, and the deterministic id makes that
 *  idempotent anyway. */
export type SeedLedger = Record<string, string[]>

/** Read one organisation's entry, defensively — the ledger arrives
 *  from storage a person can edit, so anything that is not a list of
 *  strings reads as "nothing has been seeded yet". */
export function seededIds(ledger: SeedLedger, orgId: string): string[] {
  const list: unknown = ledger[orgId]
...
/** The two writes seeding performs, handed in by the caller.
 *
 *  `register` is `constraintDefs.registerConstraints` and `remember`
 *  is the ledger write; both used to happen inside this file against
 *  a module registry and that same browser store. They are callbacks
 *  so the rules stay pure and so a caller can put the ledger where it
 *  keeps the organisation — today the repository, tomorrow Postgres.
 *  Neither is optional: a seed offered and not remembered would be
 *  offered again on the next load, which is the duplicate the
 *  deterministic id exists to prevent. */
export interface WorkbookSeedWrites {
  register: (fresh: ConstraintDef[]) => void
  remember: (orgId: string, ids: string[]) => void
}

/**
 * Put the workbook's rules in front of an organisation, once.
 *
 ...
 *
 * THE STORE READ IS A PARAMETER. `useProjectStore.getState()` gave it
 * the organisation and the tables; `CatalogueCtx` carries both, and a
 * test can hand it exactly the project it means.
 */
export function seedWorkbookConstraints(
  ctx: CatalogueCtx,
  ledger: SeedLedger,
  writes: WorkbookSeedWrites,
): SeedReport {
  const report: SeedReport = { seeded: [], alreadySeeded: [], waitingForColumns: [], blocked: [] }

  const orgKey = ctx.orgId
  const done = new Set(seededIds(ledger, orgKey))

  const buildable = buildWorkbookConstraints(ctx.entities, ctx.now(), orgKey)
  ...
  if (fresh.length > 0) {
    writes.register(fresh)
    writes.remember(orgKey, [...done, ...report.seeded])
  }
 ...
 *
 *  It returns the NEXT ledger rather than writing one, for the reason
 *  in `SeedLedger`'s note: the storage is the caller's. The same
 *  ledger comes back untouched when the organisation was never
 *  seeded, so a caller can write unconditionally without a second
 *  read deciding whether it has to. */
export function forgetWorkbookSeeds(ledger: SeedLedger, orgId: string): SeedLedger {
  if (!(orgId in ledger)) return ledger
  const next = { ...ledger }
  delete next[orgId]
  return next
}
