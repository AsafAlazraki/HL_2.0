...
import { TABLE_KINDS } from '@/domain/model'
import type { ConstraintDef, TableKind } from '@/domain/model'
...
export function relatablePairs(project: DiscoveryProject): RelatablePair[] {
  ...
  return out.sort((a, b) => b.pairings - a.pairings || b.partner.catalogue - a.partner.catalogue)
}
 ...
 *
 * `orgId` is the contract's, not this module's: every persisted record
 * in HL_2.0 carries the tenant key, and a draft minted without one
 * could not be filed. It defaults to the honest empty `makeCtx` uses,
 * so a caller that has named no organisation drafts a rule that says
 * so rather than one claiming to belong to somebody.
 */
