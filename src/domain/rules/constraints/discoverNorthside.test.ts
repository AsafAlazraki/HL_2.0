/* ============================================================
   ...
   from the pack on every run — 53 tables, 15,691 rows — because a
   rule that rots silently is a green build and a wrong answer on a
   customer's quote. (The row count is the one the pack's manifest
   carries and the one the first assertion below reads; the "11,116"
   this line used to quote was the seed at an earlier scale.)
   ============================================================ */
...
import { DISCONTINUED_FIELD_ID, isPairFieldId, isSystemFieldId } from '@/domain/model'
...
import {
  ...
  marqueVocabulary,
  type FitmentProject,
} from '@/domain/fitment/trailerFitment'

/* ---------------------------------------------------------- */
/* The seed, and one run over it                               */
/* ---------------------------------------------------------- */

/* THE PACK IS WHERE THE SEED LIVES NOW. `buildNorthsideProject()` was a
   4 MB TypeScript literal; `loadPack()` reads the generated JSON off the
   disk once per worker. The two shapes a discovery run needs — the
   tables and their rows — are the same shapes, so every figure below is
   the same figure over the same 53 tables and 15,691 rows. */
const seed = await loadPack()
...
/**
 * THE SAME FILE UNDER DIFFERENT KEYS — what a second load used to be
 * for free, built on purpose now that the pack's ids are stable.
 *
 * Every table, column and row is renamed by its REVERSE ordinal, so
 * the ids sort in the opposite order to the ones they replace, and
 * `reference` cells are rewritten to the row's new id so the joins
 * still resolve. Nothing else about the file moves: the same tables,
 * the same rows, the same values, the same order within each list.
 */
/** THE SYSTEM COLUMNS KEEP THEIR LITERAL IDS, and this is the part a
 *  re-key gets wrong first. `__uid`, `__discontinued` and the three
 *  pair columns are read BY ID and never by name, so renaming one is
 *  not a re-key, it is a data change: renaming `__discontinued`
 *  silently un-retires every discontinued row, and the accessory
 *  catalogue this suite measures against went from 2,860 live rows to
 *  3,587 before the predicate said so. */
const literalFieldId = (id: string): boolean =>
  isPairFieldId(id) || isSystemFieldId(id) || id === DISCONTINUED_FIELD_ID

function rekeyed(p: FitmentProject): FitmentProject {
  const tables = Object.values(p.entities)
  const at = (total: number, i: number): string => `x${(total - i).toString().padStart(7, '0')}`

  const idOf = new Map<string, string>()
  tables.forEach((e, i) => idOf.set(e.id, at(tables.length, i)))
  const fieldOf = new Map<string, string>()
  let f = 0
  const fieldTotal = tables.reduce((n, e) => n + e.fields.length, 0)
  for (const e of tables) {
    for (const field of e.fields) {
      fieldOf.set(field.id, literalFieldId(field.id) ? field.id : at(fieldTotal, f))
      f += 1
    }
  }
  const rowOf = new Map<string, string>()
  let r = 0
  const rowTotal = Object.values(p.rowsByEntity).reduce((n, rows) => n + rows.length, 0)
  for (const e of tables)
    for (const row of p.rowsByEntity[e.id] ?? []) {
      rowOf.set(row.id, at(rowTotal, r))
      r += 1
    }

  /** which columns hold a row id, so only those cells are rewritten */
  const refField = new Set<string>()
  for (const e of tables)
    for (const field of e.fields) {
      if (field.type === 'reference') refField.add(field.id)
    }
  ...
  for (const e of tables) {
    ...
    entities[id] = {
      ...
      fields: e.fields.map((field) => ({
        ...field,
        id: fieldOf.get(field.id)!,
        ...(field.refEntityId
          ? { refEntityId: idOf.get(field.refEntityId) ?? field.refEntityId }
          : {}),
      })),
      ...(e.hierarchy ? { hierarchy: e.hierarchy.map((h) => fieldOf.get(h) ?? h) } : {}),
      ...(e.displayFieldId
        ? { displayFieldId: fieldOf.get(e.displayFieldId) ?? e.displayFieldId }
        : {}),
    }
    rowsByEntity[id] = (p.rowsByEntity[e.id] ?? []).map((row) => {
      const values: RowData['values'] = {}
      for (const [fieldId, value] of Object.entries(row.values)) {
        const key = fieldOf.get(fieldId) ?? fieldId
        values[key] =
          refField.has(fieldId) && typeof value === 'string' ? (rowOf.get(value) ?? value) : value
      }
      return { ...row, id: rowOf.get(row.id)!, entityId: id, values }
    })
  }
...
describe('1 · it rediscovers the series banner, and rates it as the selector', () => {
  const banner = all.find(
    (c) =>
      ...
      c.statement ===
        'A trailer is only offered with a boat whose own identity its “Series” names.',
  )
...
describe('4 · it proposes no trailer length rule', () => {
  ...
  it('DISAGREES about “Boat Size (Mtr)”, and the disagreement is the finding', () => {
    ...
    expect((size?.excluded.left.untestable ?? 0) + (size?.excluded.right.untestable ?? 0)).toBe(407)
...
describe('6 · the motor still does not settle the rigging kit', () => {
  const fd = all.find(
    (c) =>
      ...
      c.statement ===
        'Choosing the Motor (the motor) settles the Rigging Kit Option (the accessory).',
  )
...
describe('the invariants hold over all 400-odd candidates', () => {
  ...
  it('is deterministic — a second run over the same file agrees exactly', () => {
    ...
    const strip = (c: Candidate): string =>
      `${c.id}|${c.hits}/${c.tested}|${c.verdict}|${c.admitted}`
  ...
  it('is deterministic across LOADS, not just across calls — different ids, same answer', () => {
    /* THE TEST ABOVE PROVES THE WRONG THING, AND THIS IS THE ONE THAT
       ...
       It was not the same twice. `buildNorthsideProject` minted every
       entity, field and row id with `newId()` (nanoid,
       northside.ts:22767), so a second load of the identical workbook
       carried entirely different ids — which is also what an
       IndexedDB reload gave you. `inOrder` used to sort on that id,
       ...
       THE PACK MINTS THE SAME IDS EVERY TIME, so a second `loadPack()`
       cannot supply the different-ids half of this test any more —
       and loading it twice would assert nothing. The second load is
       therefore RE-KEYED here: every table, column and row is given a
       new id in REVERSE ordinal order, so the sort order of the ids
       is inverted rather than merely changed, and every reference is
       rewritten to follow. No RNG, so a failure reproduces exactly.
       The file is identical; only its keys are not.

       Compared on STATEMENT, never on id: the ids are supposed to
       differ here. That is the whole point. */
    const projectB = rekeyed(project)
    expect(Object.keys(projectB.entities)).not.toEqual(Object.keys(project.entities))
