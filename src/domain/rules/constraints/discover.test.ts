...
import { PAIR_RECOMMENDED_FIELD } from '@/domain/model'
...
function table(
  ...
  return {
    id,
    /* the tenant key the contract requires on every persisted record;
       a fixture that names no organisation carries the honest empty */
    orgId: '',
    name,
    ...(kind ? { kind } : {}),
    role,
...
const find = (
  report: { proposals: Candidate[]; notProposed: Candidate[] },
  part: string,
): Candidate | undefined =>
...
describe('the run', () => {
  ...
  it('yields as a generator, and returns the same report as draining it', () => {
    ...
    expect(step.value.proposals.length).toBe(
      discover(hullAndCradle(), { minTested: 1 }).proposals.length,
    )
  })
