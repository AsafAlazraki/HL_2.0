...
import { CAME_IN, LEFT_OUT, RATE_COMMITMENT, leftOutArtefacts, leftOutSubstantive } from './leftOut'
import { WORKBOOK_RULES } from './workbookRules'

/* The seed is the pack now — the same tables, read off the generated
   JSON once per worker. */
...
describe('the register of what was left out', () => {
  ...
  it('says what DID come in, and names a real table so the count is read', () => {
    ...
    const seeded = new Set(pack.entities.map((e) => e.name.trim().toLowerCase()))
