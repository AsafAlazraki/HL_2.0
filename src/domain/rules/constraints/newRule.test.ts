...
/* ============================================================
   PORTED 2026-09-17 FROM `features/constraints/newRule.test.ts`,
   whole: nineteen cases, every assertion unchanged, and the only
   edits are the import paths and the `orgId` the contract now
   requires on a table and on a rule.

   It is the guard `create.test.ts`'s own header names from the other
   direction — there, the app composing a sentence out of a citation;
   here, out of the first column it found. Both halves are in the tree
   now, and the reference in that header is live again.
   ============================================================ */
import { describe, expect, it } from 'vitest'
import type { ConstraintDef, EntityDef, FieldDef, FieldType, TableKind } from '@/domain/model'
...
const draft = (): ConstraintDef => ({
  ...
  /* the tenant key the contract now requires on every persisted
     record. Stricter typing, never a changed assertion — nothing
     below reads it. */
  orgId: 'northside',
  kind: 'implies',
...
describe('missingChoice — the refusal beside the button', () => {
  ...
  it('then asks for the column the rule sets', () => {
    ...
    d = {
      ...
      if: {
        ...d.if,
        clauses: [{ ...d.if.clauses[0], right: { kind: 'literal', value: 'X' } }],
      },
    }
