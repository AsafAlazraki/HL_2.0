...
/* THE PACK IS WHERE THE SEED LIVES NOW. `buildNorthsideProject()` was
   a 4 MB TypeScript literal; `loadPack()` reads the generated JSON off
   the disk once per worker. The two shapes a discovery run needs — the
   tables and their rows — are the same shapes, so the report measured
   below is the same report over the same 53 tables and 15,691 rows.
   The pack's own insertion order stands in for "seed order"; what this
   suite asserts is that the engine does not care either way. */
const pack = await loadPack()

function projectOf(): DiscoveryProject {
  return {
    entities: Object.fromEntries(pack.entities.map((e) => [e.id, e])),
