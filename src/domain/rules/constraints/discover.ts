...
import type { EntityDef, FieldDef, RowData, TableKind } from '@/domain/model'
import type { FitmentProject } from '@/domain/fitment/trailerFitment'
import {
  Exclusions,
  emptyExclusions,
  exact,
...
export const THRESHOLDS = {
  filter:
    'a rule is admissible as a filter at ≥ 99 % on the price file’s own pairings, and it must reject something',
  warning: 'a rule is admissible as a warning at ≥ 95 % with a named, countable exception set',
  observation:
    'an observation is reportable at any rate, stated with its numerator and its denominator',
  floor:
    'a rule that keeps 95 % of the catalogue has not selected anything — it is a floor, not a selector',
  vacuous:
    'a bound no row in the catalogue could ever violate is arithmetic, not a rule — it leaves 100 % of the catalogue standing for every row measured',
  recommendation:
    'a rule that rejects the business’s own recommendation is REJECTED OUTRIGHT — the A2 failure, and it does not get made twice',
  deletes:
    'a candidate that would delete rows the price file itself wrote is REJECTED OUTRIGHT, and says how many, by name',
  joinKey:
    'a join key is admissible at ≥ 98 % exact match with the far-side key column ≥ 99 % unique',
...
function groupVocabulary(ctx: Ctx, kind: TableKind): Group[] {
  const candidates = new Map<
    string,
    { name: string; from: 'table' | 'level'; tables: Set<string> }
  >()
  ...
  if (levelId) {
    ...
    if (typeof v === 'string' && v.trim() !== '') {
      const hit = groups.find(
        (g) =>
          g.from === 'level' &&
          g.tableId === side.entity.id &&
          fold(g.name).trim() === fold(v).trim(),
      )
      if (hit) return hit
    }
  }
...
function selectorCandidates(sc: ShapeCtx): Candidate[] {
  ...
  for (const [subjectSide, partnerSide] of [
    ...
    for (const col of sc.ctx.columns(partnerKind, ['text', 'select'])) {
      ...
      if (populated === 0 || distinct.size < 2 || distinct.size > Math.max(2, populated / 2))
        continue
      ...
      const colGroups = groups.filter((g) => [...distinct].some((v) => names(v, g.name)))
 ...
 */
/* `_ctx` is unread and kept: the signature says what this reads from,
   and `noUnusedParameters` is on in HL_2.0 where it was not before. */
function joinTextSegments(_ctx: Ctx, join: EntityDef, joinRow: RowData): Set<string> {
...
function* joinKeySteps(
  ...
  const refsByKind = new Map<
    TableKind,
    Array<{ join: EntityDef; joinRow: RowData; target: SideRow }>
  >()
  for (const join of Object.values(ctx.project.entities)) {
  ...
  for (const combo of combos) {
    ...
    out.push({
      ...
      uniqueness: {
        ...
        duplicated: deletes,
        examples: repeats
          .slice(0, sc.opts.maxCounterExamples)
          .map((c) => ({ value: c.label, rows: c.n })),
        exampleTotal: repeats.length,
      },
...
export function* discoverSteps(
  ...
  const relationships = relationshipsOf(ctx).filter((r) => !wanted || wanted.includes(r.key))
  ...
  return {
    ...
    bounds: {
      ...
      restated: restated.n,
      withheld:
        proposals.length - shownProposals.length + (notProposed.length - shownNotProposed.length),
    },
    ms: Date.now() - started,
  }
}
