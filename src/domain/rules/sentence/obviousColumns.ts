 ...
 */
export function obviousSay(source: ObviousColumn | null, match: ObviousColumn | null): string {
  ...
  const head = firm.length > 0 ? `${firm.map((p) => p.field.name).join(' and ')}, and ` : ''
...
export function obviousLabel(source: ObviousColumn | null, match: ObviousColumn | null): string {
  const picks = [source, match].filter((p): p is ObviousColumn => p !== null)
