/* ============================================================
   ...
   PORT NOTE (HL_2.0). Two mechanical changes, no arithmetic touched.

   THE DRAG EVENT IS A SHAPE, NOT REACT'S. The old signatures took
   `React.DragEvent<Element>`, and `src/domain` imports no React. The
   four members this file actually uses are written out below as
   `PaletteDrag` — which is also exactly what the test's fake
   DataTransfer already implemented, so nothing about what is
   asserted moved.

   THE STORE IS A PARAMETER. `useProjectStore.getState()` read the
   rules map and called two mutators. The read is `ctx.rules` off a
   `CatalogueCtx`, and the two writes are callbacks the caller hands
   in — the same `(ctx, writes, …)` shape `views/pairs.ts` takes.
   ============================================================ */

import { RULE_NODE_KINDS, type CatalogueCtx } from '@/domain/model'
import type { RuleDef, RuleEdge, RuleNode, RuleNodeKind, XY } from '@/domain/model'

export const RULE_DND_MIME = 'application/x-helmlogic-rule-node'

/** What this file needs of a drag — the browser's own `DragEvent`
 *  satisfies it, and so does React's synthetic one. Written out
 *  rather than imported because a domain module knows no DOM and no
 *  React; these are the only four members the three readers below
 *  touch. */
...
/** The two writes this file makes, handed in by the caller that owns
 *  the catalogue. Both keep the store's own return: `addRuleNode`
 *  answers null for a rule that is not there, and `connectRuleNodes`
 *  answers the edge it wrote or reused. */
export interface RuleCanvasWrites {
  addRuleNode: (ruleId: string, kind: RuleNodeKind, position: XY) => RuleNode | null
  connectRuleNodes: (
    ruleId: string,
...
/** What this file reads of the world: the rules, by id. */
export type RuleCtx = Pick<CatalogueCtx, 'rules'>
...
export function setPaletteDragData(event: PaletteDrag, kind: RuleNodeKind): void {
...
export function isPaletteDrag(event: PaletteDrag): boolean {
  return Array.from(event.dataTransfer.types).includes(RULE_DND_MIME)
}

export function readPaletteKind(event: PaletteDrag): RuleNodeKind | null {
  const raw = event.dataTransfer.getData(RULE_DND_MIME) || event.dataTransfer.getData('text/plain')
...
export function onPaletteDrop(
  ...
  return addRuleNodeAt(ctx, writes, ruleId, kind, position)
}
...
export function addRuleNodeAt(
  ...
  return writes.addRuleNode(ruleId, kind, clearOf(rule, want))
}
...
export function nextNodePosition(ctx: RuleCtx, ruleId: string): XY {
  const rule = ctx.rules[ruleId]
...
export function addRuleNodeAfter(
  ...
  const node = writes.addRuleNode(ruleId, kind, clearOf(rule, want))
  if (!node) return null
  ...
  if (canWire && anchor) {
    writes.connectRuleNodes(ruleId, { source: anchor.id, target: node.id })
  }
  return node
}
