...
export const kindInk = (k: RuleNodeKind): string => `var(--accent-${KIND_ACCENT[k]})`

/** Canvas ink (reads on the navy blueprint). */
export const kindInkBright = (k: RuleNodeKind): string => `var(--accent-${KIND_ACCENT[k]}-bright)`
...
export function opsForType(type: FieldType | undefined): CompareOp[] {
  switch (type) {
    ...
    default:
      return [
        ...NUMERIC_OPS,
        ...TEXT_OPS.filter((o) => !NUMERIC_OPS.includes(o)),
        'isTrue',
        'isFalse',
      ]
  }
...
export function describeAction(
  ...
  switch (action.op) {
    ...
    case 'link': {
      ...
      return {
        title: 'Link',
        summary: ok ? `This pair into ${join.name}` : `${join.name} — wire up both sides`,
        configured: ok,
      }
    }
    default:
...
function actionPlate(
  ...
  switch (action.op) {
    ...
    case 'create': {
      const target = entities[action.entityId]
      if (!target)
        return { subject: 'Create', lines: [miss('cr', 'pick a table to create a row in')] }
      return {
        subject: 'Create',
        lines: [
  ...
  switch (node.kind) {
    case 'start':
      return source
        ? {
            subject: source.name,
            lines: [{ id: 'walk', chunks: [word('walk every'), stampChunk(source), word('row')] }],
          }
    ...
    case 'match': {
      const target = entities[node.config.targetEntityId]
      const footer =
        node.config.emptyBehavior === 'passThrough' ? 'no match → carry on' : 'no match → skip row'
...
export const typeTag = (f: FieldDef | undefined): string => (f ? FIELD_TYPES[f.type].tag : '')
