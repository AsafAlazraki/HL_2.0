...
export type TokenType =
  'number' | 'string' | 'field' | 'ident' | 'op' | 'lparen' | 'rparen' | 'comma' | 'eof'
...
export function tokenize(src: string): Token[] {
  ...
  while (i < n) {
    ...
    if (c === '[') {
      ...
      if (!closed) {
        throw new FormulaError(`Missing ']' for the field reference at ${posLabel(start)}`, start)
      }
      name = name.trim()
      if (!name) {
        throw new FormulaError(`Empty field reference [] at ${posLabel(start)}`, start)
      }
