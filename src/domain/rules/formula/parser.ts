...
import { FormulaError, posLabel, type BinaryOp, type Expr } from './ast'
...
export function parse(src: string): Expr {
  ...
  function expectRParen(openerLabel: string, openPos: number): void {
    ...
    if (t.type === 'eof') {
      throw new FormulaError(`Missing ')' to close ${openerLabel} at ${posLabel(openPos)}`, t.pos)
    }
  ...
  const tail = peek()
  if (tail.type !== 'eof') {
    throw new FormulaError(`Unexpected token '${tail.text}' at ${posLabel(tail.pos)}`, tail.pos)
  }
  return expr
}
