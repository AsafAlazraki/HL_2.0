...
import type { CellValue, ImageRef } from '@/domain/model'
 ...
 *  (its `types` is vite/client alone), and the old repo already reached
 *  host globals through a cast on globalThis — its
 *  features/history/render.test.ts:45. Used only by the two time-zone
 *  tests below, which restore what they set. */
...
describe('SUM', () => {
  ...
  it('refuses to overflow to Infinity mid-total', () => {
    ...
    expect(() => value('SUM(10 ^ 308, 10 ^ 308)')).toThrow("Result of 'SUM' is not a valid number")
  })
...
describe('ROUND — the places argument', () => {
  ...
  it('refuses to produce Infinity', () => {
    ...
    expect(() => value('ROUND(10 ^ 308, 2)')).toThrow("Result of 'ROUND' is not a valid number")
  })
...
describe('YEAR / MONTH / DAY', () => {
  ...
  it('names the function in the message, and empty separately from wrong', () => {
    ...
    expect(() => value('YEAR("nope")')).toThrow('YEAR: "nope" is not a date (expected YYYY-MM-DD)')
...
describe('DATEDIFF', () => {
  ...
  it('reports which end was not a date, and needs both', () => {
    ...
    expect(() => value('DATEDIFF("2024-01-01")')).toThrow('DATEDIFF expects 2 arguments, got 1')
    expect(() => value('DATEDIFF("a", "b", "c")')).toThrow('DATEDIFF expects 2 arguments, got 3')
  })
})
