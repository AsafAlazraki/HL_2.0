...
import { placesOf } from '@/domain/modules/places'
import type { ConstraintDef, EntityDef, RowData } from '@/domain/model'
...
import { buildSteps, HANDOVER_STEP, SUBJECT_STEP } from './steps'
import {
  SUBJECT_CAP,
  andList,
  catalogueOf,
...
import { placeRules, subjectVerdict } from './subjectRules'
...
/* THE PACK IN PLACE OF THE SEED, AND THE CONTEXT IN PLACE OF THE
   STORE. `loadNorthsideProject()` replaced the project in the store
   and minted the nine places on top of it; there is no store now, the
   sheet is the pack, and the places are on the context — see the
   longer note in `bands.test.ts` for why this file does not mint them
   itself. */
...
describe('the walk a subject opens is drawn before it is taken', () => {
  ...
  it('says so when a subject is related to nothing, rather than drawing two stops in silence', () => {
    ...
    const lonely = Object.values(entities).find(
      (e) => e.role !== 'join' && flowPreview(e, entities, []).stops.length === 2,
    )
    if (!lonely) return
...
function expectPreviewMatchesQuote(place: string): void {
  ...
  const view = createViewFor(ctx, subject.entry.tableId)
  const views = Object.values(ctx.views)
  const preview = flowPreview(subject.entity, entities, views)
...
describe('what the rules already decide about one row', () => {
  ...
  it('narrows another column and carries the rule’s own reason, verbatim', () => {
    ...
    const rule: ConstraintDef = {
      id: 'test-narrowing',
      /* the tenant key the contract requires of every record; this
         rule is written against the pack's own sheet */
      orgId: 'northside',
      kind: 'excludes',
      if: {
        combinator: 'AND',
  ...
  it('never throws, whatever it is handed', () => {
    ...
    const broken: ConstraintDef = {
      ...
      if: {
        combinator: 'AND',
        clauses: [{ id: 'x', left: { fieldId: 'no-such-column' }, op: 'isEmpty' }],
      },
