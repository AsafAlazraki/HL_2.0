...
/* THE DIRECT PATH, AND IT IS NOT A STYLE CHOICE. `modules/read.ts`
   ...
   `@/domain/catalogue/views/sellable` two lines down. */
import {
  buildEntries,
  censusLine,
  groupEntries,
  ...
  type IndexSection,
  type ModuleCensus,
} from '@/domain/modules/read'
import { moduleAt, placesOf } from '@/domain/modules/places'
import { defaultBlocksFor } from '@/domain/catalogue/views/relations'
import { retiredTablesSentence } from '@/domain/catalogue/views/sellable'
...
export function quoteDoors(
  ...
  return placesOf(byId, entities, rowsByEntity).map((place) => {
    ...
    const module =
      place.tableId === undefined ? base : { ...moduleAt(base, place.tableId), name: place.name }
    ...
    return {
      ...
      module,
      tables: listed,
      kind: primary && primary.kind && primary.kind in TABLE_KINDS ? primary.kind : 'custom',
      census,
...
export function flowPreview(
  ...
  const blocks: ViewBlock[] = existing ? existing.blocks : defaultBlocksFor(entities, entity.id)
