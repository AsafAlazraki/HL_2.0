...
/* ============================================================
   THE CASE THAT CANNOT RUN YET, WRITTEN DOWN RATHER THAN DROPPED.

   The old suite opened with one case this port cannot make — "the
   board and the list find the same things", asserting that the
   board's `matches` (the re-export in `features/pipeline/finding`)
   and this file's `matches` are the same function object, not two
   that agree today.

   It pins the thing a second pair of eyes would undo: the pipeline
   board and the quotes list calling ONE matcher rather than two that
   agree today. The board is not part of Milestone 0 — the whiteboard
   and the pipeline are deliberately deferred (docs/PLAN.md, Milestone
   5) — so the module it imports does not exist here and the identity
   cannot be asserted against anything. It is not weakened and it is
   not deleted: it goes back, verbatim, the day the board is ported,
   and until then it is listed as a blocker rather than as a pass.
   ============================================================ */
