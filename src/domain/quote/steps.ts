 ...
 *
 *  THE LITERAL MOVED TO THE CONTRACT ON THE WAY IN, and this is the
 *  same string rather than a second one: `SUBJECT_CHAPTER` is where
 *  the model now declares it, because the quote carries its chapters
 *  and a chapter and a step name one stop. The export stays so every
 *  reader of this file keeps its own word for it. */
export const SUBJECT_STEP = SUBJECT_CHAPTER
 ...
 *  now name it: the configurator walks to it, the remembered place
 *  points at it across a reload, and `start.ts` previews it as the
 *  end of the walk before a document exists. Three copies of a magic
 *  string is three chances for a remembered place to point at a stop
 *  nothing matches — which is why the string itself is the contract's
 *  `HANDOVER_CHAPTER` and this is its name here. */
export const HANDOVER_STEP = HANDOVER_CHAPTER
 ...
 *  and `@/domain/catalogue/curation` prints the same clause three inches
