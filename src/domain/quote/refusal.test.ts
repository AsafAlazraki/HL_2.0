...
/* ============================================================
   WHAT THIS PORT COULD NOT DECIDE, AND HAS NOT DECIDED.

   Every assertion below is the one this suite already made. Two of
   them read fields the CONTRACT no longer carries: the old
   `Candidate` marked a row reached past the narrowing with
   `outside: true` and `outsideWhy: <a sentence>`, and
   `domain/model/offer.ts` replaces both with facts — `standing:
   'outside'` and a `verdicts` list whose members carry the figures
   the decision turned on, so the sentence is composed once, at the
   surface that prints it.

   THAT IS THE BETTER SHAPE AND IT IS NOT THIS FILE'S TO IMPOSE. The
   assertions stay as they were written until `freeze.ts` lands and
   the two are reconciled deliberately: the claims — a reached-past
   row says WHY, the sentence names real cells, an offered row says
   nothing — are true of either shape, and re-expressing them against
   `verdicts` is a decision for whoever ports `stepOffer`, never a
   mechanical change a port may make on its own.
   ============================================================ */
...
import { singular } from '@/domain/catalogue/views/describe'
import { mintQuoteFromView, stepOffer, type Candidate } from './freeze'
import { SUBJECT_STEP } from './steps'

/* The pack in place of the seed, and the context in place of the
   store — see the note in `steps.test.ts`. */
  ...
  for (const boat of boats) {
    ...
    for (const row of (pack.rowsByEntity[boat.id] ?? []).slice(0, 8)) {
      ...
      for (const section of quote.sections) {
        ...
        const offer = stepOffer(ctx, quote, section, { all: true })
...
describe('a row the narrowing left out says why, on itself', () => {
  ...
  it('names the list that records the pairing, when that is the reason', () => {
    ...
    const joins = pack.entities.filter((e) => e.role === 'join').map((e) => e.name)
...
describe('nothing in the sentence is invented', () => {
  it('quotes only figures the two rows actually hold', () => {
    ...
    const rootRow = (pack.rowsByEntity[reach.quote.rootTableId] ?? []).find(
    ...
    for (const c of reach.outside) {
      ...
      /* Every bracketed figure the sentence prints must be readable
         ...
         provenance. */
      const row = (pack.rowsByEntity[reach.section.tableId] ?? []).find(
        (r) => r.id === c.line.rowId,
      )
  ...
  it('quotes BOTH sides of a measurement when the narrowing is a rule', () => {
    ...
    for (const b of boats) {
      ...
      if (!hull) continue
      for (const block of createViewFor(ctx, b.id).blocks) {
    ...
    const row = (pack.rowsByEntity[boat.id] ?? []).find(
      (r) => typeof r.values[hullLen.id] === 'number',
    )
    expect(row).toBeTruthy()
    if (!row) return
    const quote = mintQuoteFromView(ctx, { viewId: view.id, rowId: row.id, reference: 'T-2' })
    ...
    const rowsOf = pack.rowsByEntity[block.tableId] ?? []
