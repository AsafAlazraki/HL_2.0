...
import { PAIR_ORDER_FIELD, readCell } from '@/domain/model'
import type { CatalogueCtx, EntityDef, RowData } from '@/domain/model'
import { loadPack } from '@/test/fixtures/pack'
import { joinRefFor, makeEngine } from '@/domain/catalogue/views'
import { mintLine } from './freeze'
import { defaultLevelKey } from './pricing'

/* ---------------------------------------------------------- */
/* One boat × motor pairing out of the real seed              */
/* ---------------------------------------------------------- */

/* THE PACK IN PLACE OF THE SEED, AND ONE CONTEXT IN PLACE OF TWO.
   The old suite mocked the repository so the store would not hydrate,
   built the sheet with `buildNorthsideProject()` and handed the view
   engine the two-field `Ctx` the views feature declared for itself.
   The pack is the same sheet, and `CatalogueCtx` is that shape
   widened to every field a ported module reads — so the engine, the
   join reader and `mintLine` are all handed one world. */
const project = await loadPack()
const entities: Record<string, EntityDef> = project.ctx.entities
const ctx: CatalogueCtx = project.ctx
