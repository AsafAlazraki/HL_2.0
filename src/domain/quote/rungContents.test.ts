...
import type { FrozenLevel } from '@/domain/model'
...
import { CHARGE_TITLE } from '@/domain/model'

/* THE SEEDED TABLES ARRIVE FROM THE PACK. The old suite built them
   with `buildNorthsideProject()` and mocked the repository so the
   store would not hydrate underneath it; the pack reads the same 53
   tables off `data/northside/` and there is no store to stop. The
   tables it asks for are the same tables, and — this is the half
   that matters here — the pack DECLARES each one's ladder
   (`EntityDef.priceLevels`) with the same four `contains` facts and
   the same four cells, so `priceLevelsFor` answers off the
   declaration rather than off the name list and every assertion
   below is still about the file. */
