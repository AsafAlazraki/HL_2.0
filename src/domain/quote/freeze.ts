/* ============================================================
   ...
   drawn. That separation is the whole correctness story:
     ...
     · freeze.ts   reads the catalogue, ONCE, and produces frozen values
     · totals.ts   sums frozen values and never reads the catalogue
     · the screens render frozen values and never read the catalogue

   THE ONE MECHANICAL CHANGE THIS PORT MAKES, AND WHY IT IS THE ONLY
   ONE. The file this came from opened fourteen events with
   `useProjectStore.getState()` and six more with a `live()` helper
   that wrapped it. `src/domain` is pure — no React, no store, no DOM
   — so every one of those reads is now the SAME snapshot, handed in:
   a `CatalogueCtx` (`@/domain/model/ctx.ts`) as the first argument of
   every exported entry point, with `ctx.now()` as the clock and
   `ctx.views` in place of the module-level view registry the plan
   drops by name. Not one line of reasoning below moved, and not one
   figure is derived differently: a snapshot taken at the top of an
   event is exactly what `live()` was, with the taking moved out to
   the caller that owns the event.

   `fileCustomer` is the single exception and it is not a read: it
   ADDS A ROW, so the act arrives as a callback beside the context,
   the same treatment `pairs.ts` gives its two writes.
...
import {
  ...
  type JoinRef,
  type RelatedRow,
} from '@/domain/catalogue/views'
/* DEEP, for the same reason `people/customers` is deep below: `describe`
...
import { plural, ruleReason, thisOne } from '@/domain/catalogue/views/describe'
/* THE MEASUREMENTS, READ AND NEVER TYPED. `ruleLedger` imports only
   `@/domain/model` and one table name; it touches no store and no React,
...
import { holdRate, ledgerFor } from '@/domain/rules/constraints/ruleLedger'
import {
  bandOf,
  defaultColumns,
  formatCell,
  ...
  rangePairs,
  splitUnit,
} from '@/domain/catalogue/views/columns'
/* DEEP IMPORT, DELIBERATELY. The old `@/features/crm` barrel drew
   customer screens that read quotes, so reaching for a barrel here
   would close that circle. `people/customers` is the pure half — no
   store, no React, no import of ours — and importing the leaf rather
   than the package is the move that kept it open. */
import {
...
/* ---------------------------------------------------------- */
/* Reading the catalogue, once                                */
/* ---------------------------------------------------------- */

/* WHAT `live()` WAS. It read `useProjectStore.getState()` and built a
   two-map `Ctx` and a rule engine from it, at the top of every pick:
   "Deliberately NOT a hook and NOT memoised: every caller here is an
   event, not a render." The snapshot is now handed in — that is the
   whole of the change — so what is left of `live()` is the engine,
   which is derived from the snapshot and is built exactly where it
   was built before. A `CatalogueCtx` satisfies the two-map `Ctx` the
   view functions take, so nothing is converted on the way through. */
 ...
 *
 *  IT IS THE CONTRACT'S LITERAL NOW, not a second declaration of the
 *  same string: chapters ARE the quote's sections (docs/DECISIONS.md)
 *  and `SUBJECT_CHAPTER` in `@/domain/model` is where the subject's
 *  id is declared, for the section, the chapter and the step alike.
 *  `steps.ts` already re-exports it as `SUBJECT_STEP` on those
 *  grounds; this is the same move, so the three names are one
 *  string and cannot drift. */
export const SUBJECT_BLOCK = SUBJECT_CHAPTER
 ...
 */
export function mintQuoteFromView(ctx: CatalogueCtx, args: MintQuoteArgs): QuoteDef | null {
  const engine = makeEngine(ctx)
  const view = ctx.views[args.viewId]
  if (!view) return null
  ...
  return {
    id: newId(),
    /* THE TENANT KEY, STAMPED FROM THE CONTEXT THE QUOTE WAS MINTED
       AGAINST. The contract requires it on every persisted record and
       it is not a decision this function may make on its own: the
       rows it froze carry it, so the document carries the same one. */
    orgId: ctx.orgId,
    ...
    sections,
    /* THE WALK, FROZEN WITH THE SECTIONS IT DRAWS. Chapters ARE the
       quote's sections (docs/DECISIONS.md), so the list is derived
       from `sections` at mint time and never re-derived from the live
       view: a page re-curated on Tuesday must not reorder the walk a
       person is in the middle of on Monday, which is the same promise
       every price on the page keeps. */
    chapters: chaptersOf(ctx, sections),
    lines,
    adjustments: [],
    /* NOTHING HAS HAPPENED TO IT YET — the mint's own event is written
       by the command that called this, beside the inverse it pushes,
       because that is where every other event on this document is
       written and two places writing events is two audits. */
    events: [],
    levelKey,
    ...
    /* THE DEALERSHIP'S STANDING TERMS, COPIED ON — adopt 10's first
       ...
       prints none. */
    ...(() => {
      const terms = ctx.org?.quoteTerms?.trim()
...
/** The walk, read off the sections the mint just wrote.
 *
 *  A SECTION CHAPTER DRAWS THE SECTION WHOSE `blockId` IT NAMES, so
 *  the two lists are the same list in the same order and the chapter
 *  adds only the table's kind — which a band head reads for its hue
 *  and which `sectionKinds` otherwise has to ask the sheet for on
 *  every redraw.
 *
 *  THE HANDOVER IS THE LAST STOP AND IT IS NOT A SECTION: no table
 *  carries "who is it for", so it is appended rather than minted as a
 *  section with nothing behind it. `buildSteps` reads SECTIONS and so
 *  never draws it as a step; the walk reads chapters and does, which
 *  is the difference the contract declares both ends for. */
function chaptersOf(ctx: CatalogueCtx, sections: readonly QuoteSection[]): QuoteChapter[] {
  const out: QuoteChapter[] = sections.map((s) => {
    const entity = ctx.entities[s.tableId]
    const kind = entity?.kind
    return {
      id: s.blockId,
      title: s.title,
      ...(s.tableId !== '' ? { tableId: s.tableId } : {}),
      ...(kind && kind in TABLE_KINDS ? { kind } : {}),
    }
  })
  out.push({ id: HANDOVER_CHAPTER, title: HANDOVER_TITLE })
  return out
}

/** The heading the handover chapter carries. In the dealer's words,
 *  and typed nowhere else: the walk's last stop asks who the quote is
 *  for, and that question has no table to name it. */
export const HANDOVER_TITLE = 'Who it is for'
 ...
 *  go?". The sentence the editor prints comes from `@/domain/catalogue/views`
...
export interface Offer {
  ...
  /**
   * THE THREE FIGURES `@/domain/catalogue/curation` NEEDS, and the reason
...
/* ============================================================
   ...
   ============================================================ */
export function refinishSubject(
  ctx: CatalogueCtx,
  quote: QuoteDef,
  rowId: string,
): QuoteDef | null {
...
function viewForQuote(ctx: CatalogueCtx, quote: QuoteDef): ViewDef | undefined {
  const stored = ctx.views[quote.viewId]
  ...
  if (!ctx.entities[quote.rootTableId]) return undefined
  return createViewFor(ctx, quote.rootTableId)
}
 ...
 */
export function candidateOffer(ctx: CatalogueCtx, quote: QuoteDef, section: QuoteSection): Offer {
  ...
  const view = viewForQuote(ctx, quote)
  if (!view) return none
...
export function candidatesFor(
  ...
  return candidateOffer(ctx, quote, section).candidates
}
...
export interface StepOffer {
   ...
   * and Max HP" — for `@/domain/catalogue/curation` to sit the word "because"
   * in front of.
   *
...
const bracket = (
  path: { fieldId: string; viaFieldId?: string },
  entity: EntityDef,
  row: RowData,
): string => {
...
function outsideWhy(args: {
  ...
  for (const c of rule.clauses) {
    const holds = evalPairRule(
      engine,
      { combinator: 'AND', clauses: [c] },
      { entityId: target.id, row },
      source,
    )
    if (holds) continue
...
export interface StepMeasure {
  ...
  caveat: string
  /**
   * THE ONE-CLAUSE FORM, for `@/domain/catalogue/curation` to carry on its
 ...
 *  `@/domain/catalogue/curation` reads. `what` is a clause with no leading
 ...
 */
export function unsellableSubject(
  ctx: CatalogueCtx,
  rootTableId: string,
  rootRowId: string,
): string {
  const { entities, rowsByEntity } = ctx
...
export const sectionTableIsGone = (ctx: CatalogueCtx, section: QuoteSection): boolean =>
  !ctx.entities[section.tableId]
 ...
 */
export function sectionKinds(ctx: CatalogueCtx, quote: QuoteDef): Record<string, TableKind> {
  const { entities } = ctx
 ...
 */
export function priceChanges(ctx: CatalogueCtx, quote: QuoteDef): PriceChange[] {
  const engine = makeEngine(ctx)
...
export function subjectStillOnSheet(ctx: CatalogueCtx, quote: QuoteDef): EntityDef | undefined {
  const { entities } = ctx
 ...
 *
 * This feature keeps the CATALOGUE to exactly one file — it was
 * `useProjectStore` and it is the injected `CatalogueCtx` now — so
 * that no drawn quote can touch live data, and a customer is live data:
 ...
 * matched — is `@/domain/people/customers`, which imports nothing
 * and reads nothing. This file supplies the only thing that file
 * refuses to: the current state of somebody's project.
 */

/** The one act this file performs, handed in. `addRow` is the
 *  catalogue's own action with its own signature; nothing here
 *  decides what a write is. */
export interface CustomerWrites {
...
export function customerBook(ctx: CatalogueCtx): CustomerRead[] {
  const { entities, rowsByEntity } = ctx
...
export const hasCustomerRegister = (ctx: CatalogueCtx): boolean =>
  customerRegister(ctx.entities) !== undefined
 ...
 *
 * THE ONE WRITE IN THIS FILE, AND IT ARRIVES AS A CALLBACK — the
 * same treatment `pairs.ts` gives its two. `src/domain` is pure, so
 * the read comes off the context and the act is the caller's own
 * `addRow`, with the signature it already has, so a caller wires it
 * by name and nothing here learned a new idea about what a write is.
 *
...
export function fileCustomer(
  ...
  const row = writes.addRow(table.id, values)
  /* THE ROW IS READ BACK OFF THE ROW ITSELF, not off a re-taken
     snapshot. `ctx` is the world as it stood at the top of this event
     and the row that has just been added is not in it, so
     `freezeCustomer` is handed a context with the new row in place —
     the one-line widening a pure module needs to read what the
     callback just wrote, and nothing more. */
  if (!row) return null
  const after: CatalogueCtx = {
    ...ctx,
    rowsByEntity: { ...ctx.rowsByEntity, [table.id]: [...(ctx.rowsByEntity[table.id] ?? []), row] },
  }
  return freezeCustomer(after, row.id)
}
