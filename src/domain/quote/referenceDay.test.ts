...
/* ============================================================
   PORTED 2026-09-17 FROM `features/quote/referenceDay.test.ts`,
   whole: six cases, every assertion unchanged.

   `nthToday` CAME ACROSS WITH IT. The old one read the registry's
   module-level `list`; the registry is `src/state/quotes.ts` now and
   `src/domain` may not read it, so the documents already held arrive
   as an argument — the port rule for a store read, applied to the one
   function this suite exists to hold to the same calendar as
   `referenceFor`. Before this file it was the only export of the old
   `quotes.ts` with nothing in the tree calling it, which is why
   nothing in the tree could mint a reference at all.

   `makeNewVersion(id)` is `newVersionOf(from, reference, now)` here:
   the registry chose the reference itself, and the pure command takes
   it, so the one line that used to be inside it — `referenceForNow()`
   — is written out in the test that measures it. That is the whole of
   the mechanical change; the two seeds, the arithmetic and the
   expected `-02` are the old file's.
   ============================================================ */
...
const { newVersionOf } = await import('./commands')
const { localDay, localDayOf } = await import('./day')
const { referenceFor, referenceForNow } = await import('./freeze')

import type { QuoteDef } from '@/domain/model'

/* ---------------------------------------------------------- */

/** The registry, which is a list now and was a module-level one
 *  before. `nthToday` counts everything in it, so a leftover quote is
 *  a leftover answer and `beforeEach` empties it. */
let list: QuoteDef[] = []
...
describe('the suffix counts the LOCAL day, which is what the stamp says', () => {
  ...
  it("the next reference is today's second, whatever the wall clock says", () => {
    ...
    const at = new Date()
    const made = newVersionOf(source, referenceForNow(list, at), at.toISOString()).quote
