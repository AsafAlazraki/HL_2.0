...
const XCB: Fact[] = [
  {
    ...
    value: 'Helm Master L2 - 6X9 Binnacle | Bolt on DES | Straight Helm | EKS | Single',
  },
  PROP_NO,
  PROP_DESC,
]
const XSB: Fact[] = [
  {
    label: 'Rigging Kit Option',
    value: 'Helm Master L2 - 6X9 Binnacle | Built in DES | Straight Helm | EKS | Single',
  },
  PROP_NO,
  PROP_DESC,
]
...
describe('inside the fact, the segment that is actually the decision', () => {
  ...
  it('keeps every segment that is this option’s own', () => {
    ...
    const out = distinguishingFacts([XCB, XSB, XSB2])
    expect(out[2][0].value).toBe(
      'Helm Master L4 (Chrome) - 6X9 Binnacle · Built in DES · Full Maneuverability',
    )
  })
