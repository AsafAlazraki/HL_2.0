...
import type { EntityDef, TableKind } from '@/domain/model'
import { loadPack } from '@/test/fixtures/pack'
import { createViewFor } from '@/domain/catalogue/views'
import { mintQuoteFromView, sectionKinds } from './freeze'
...
/* ============================================================
   ...
   The old suite loaded the seed INTO the store — `loadNorthsideProject()`
   replaced the project and then minted the nine places through the
   store's own `createModule` — and mocked the repository so Dexie
   would not write underneath it. There is no store here: the sheet is
   the pack, the world a reader needs is a `CatalogueCtx`, and the
   places are on it.

   THE PLACES ARE THE ONE THING THIS FILE CANNOT MINT FOR ITSELF. They
   are app code, not data (docs/PLAN.md: "the nine places are minted
   by app code from table keys"), and inventing nine module records
   here would be exactly the fake data the house rule forbids — the
   names, the descriptions and the verbs are the dealer's. So they are
   read off the context, and until the minting lands `ctx.modules` is
   empty and this suite says so by failing to find a door rather than
   by testing a fixture.
   ============================================================ */
...
function bandsFor(category: string): Band[] {
  ...
  const view = createViewFor(ctx, entry.tableId)
  ...
  return orderBands(buildSteps(quote!), sectionKinds(ctx, quote!))
}
...
describe('a band is a decision and not a table', () => {
  ...
  it('folds a kind’s several tables into ONE band with several headings', () => {
    ...
    const entry = catalogueOf(door, rowsByEntity)[0]
    const view = createViewFor(ctx, entry.tableId)
    const quote = mintQuoteFromView(ctx, { viewId: view.id, rowId: entry.rowId, reference: 'T-2' })!
...
describe('a quote raised on something other than a hull', () => {
  ...
  it('sorts every kind the sheet actually uses into a band', () => {
    ...
    for (const category of ['Boats', 'Motors', 'Trailers', 'Factory Packages']) {
      ...
      const drawn = orderBands(buildSteps(quote), sectionKinds(ctx, quote)).flatMap((b) =>
