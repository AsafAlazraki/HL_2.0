/* ============================================================
   ...
   IN THIS BUILD THOSE SIX FUNCTIONS ARE HANDED IN. The old file
   imported them from `@/features/quote`, where each one read the
   project store at the top of the call — `subjectStillOnSheet` and
   `unsellableSubject` read the sheet, `freezeCustomer` reads the
   register, and the other three write to the quote registry. A pure
   module may do neither, so they arrive as `AgainPorts`: the reads
   are the same functions taking the caller's snapshot, the writes are
   the caller's own callbacks. Not one line of the reasoning below
   moves, because none of it was ever about where those six live.
   ============================================================ */
 ...
 *
 *  The caller holds the `CatalogueCtx` and the quote store and binds
 *  each one before calling in; nothing here holds a snapshot across
 *  an event. The shapes are the old ones exactly — only the world
 *  they read has moved from a module-level `getState()` to the
 *  caller's hand. */
export interface AgainPorts {
  /** is the row this quote was written against still on the sheet */
  subjectStillOnSheet: (quote: QuoteDef) => boolean
  /** why this row may not be sold today, or '' when it may */
  unsellableSubject: (tableId: string, rowId: string) => string
  /** a new draft for the same row on the same page, at today's prices */
  quoteLikeThisOne: (quote: QuoteDef) => QuoteDef | null
  /** the register's row, read now, as the document would print it */
  freezeCustomer: (rowId: string) => FrozenCustomer | null
  /** write the frozen customer and the pointer onto a draft */
  linkCustomer: (quoteId: string, frozen: FrozenCustomer) => void
  /** write named fields onto a draft */
  patchQuote: (quoteId: string, patch: Partial<QuoteDef>) => void
}
...
export function whyNotAgain(quote: QuoteDef, ports: AgainPorts): string {
  ...
  const unsellable = ports.unsellableSubject(quote.rootTableId, quote.rootRowId)
 ...
 */
export function quoteAgain(quote: QuoteDef, ports: AgainPorts): QuoteDef | null {
  const made = ports.quoteLikeThisOne(quote)
  if (!made) return null

  const rowId = quote.customerRef?.rowId
  if (rowId) {
    const frozen = ports.freezeCustomer(rowId)
    if (frozen) {
      ports.linkCustomer(made.id, frozen)
      return made
    }
  }
  ...
  if (quote.customer.name.trim() !== '' || (quote.customer.contact?.length ?? 0) > 0) {
    ports.patchQuote(made.id, { customer: quote.customer })
  }
  return made
}
