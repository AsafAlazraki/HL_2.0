/* ============================================================
   ...
   looking right while the claim stopped being true.
     ...
     4 · THE COUNTS ADD UP. `pool = offered + narrowedOut + withheld`
         is the arithmetic `@/domain/catalogue/curation` prints in words. If
...
import type { EntityDef, QuoteDef, QuoteSection } from '@/domain/model'
import { loadPack } from '@/test/fixtures/pack'
import { createViewFor } from '@/domain/catalogue/views'
import { readCuration } from '@/domain/catalogue/curation'
import { holdRate, ledgerFor } from '@/domain/rules/constraints/ruleLedger'
import { mintQuoteFromView, stepOffer, stepReason } from './freeze'
import {
  buildSteps,
  decidedCount,
  firstOpenStep,
...
/* ============================================================
   ...
   Two mechanical changes and nothing else. The seed is the pack —
   the same 53 tables and 15,691 rows, read off `data/northside/` —
   so the repository mock and the store write the old suite needed
   (it set `entities` and `rowsByEntity` on `useProjectStore` before
   anything read them) are gone with the store itself. And every
   reader that used to take its world off `getState()` is handed a
   `CatalogueCtx` as its first argument, which is the convention the
   whole domain now keeps.
   ============================================================ */

const pack = await loadPack()
const ctx = pack.ctx
const entities: Record<string, EntityDef> = ctx.entities
...
function firstQuoteWithA(kind: string): { quote: QuoteDef; section: QuoteSection } | null {
  for (const boat of boats) {
    ...
    for (const row of (pack.rowsByEntity[boat.id] ?? []).slice(0, 12)) {
      const quote = mintQuoteFromView(ctx, { viewId: view.id, rowId: row.id, reference: 'T-1' })
      if (!quote) continue
...
describe('a step is a reading of a document that is already written down', () => {
  ...
  it('sums each stop from the FROZEN lines and never from the sheet', () => {
    ...
    for (const step of buildSteps(quote)) {
      ...
      expect(step.unpriced).toBe(step.lines.filter((l) => lineAmount(l).amount === null).length)
    }
  })
...
describe('a narrowing says what did it, and quotes the price file’s own rate', () => {
  it('names the list that pairs these with this one', () => {
    ...
    const why = stepReason(ctx, trailerCase.quote, trailerCase.section)
  ...
  it('carries A1 on a motor step and NOTHING on a table nobody measured', () => {
    if (motorCase) {
      const why = stepReason(ctx, motorCase.quote, motorCase.section)
    ...
    if (other && trailerCase) {
      ...
      expect(stepReason(ctx, trailerCase.quote, other)?.measured).toBeUndefined()
    }
  })
})
...
describe('every step can reach its whole table', () => {
  it('switching the narrowing off offers more than the narrowing did', () => {
    if (!trailerCase) return
    const narrowed = stepOffer(ctx, trailerCase.quote, trailerCase.section)
    const everything = stepOffer(ctx, trailerCase.quote, trailerCase.section, { all: true })
  ...
  it('the search ignores the narrowing, and marks what it reached past', () => {
    ...
    if (!outside) return
    const found = stepOffer(ctx, trailerCase.quote, trailerCase.section, {
    ...
    expect(found.candidates.find((c) => c.line.label === outside.line.label)?.outside).toBe(true)
  ...
  it('offers nothing at all for the subject — one boat on a quote for one boat', () => {
    ...
    expect(stepOffer(ctx, trailerCase.quote, subject).candidates).toEqual([])
    expect(stepReason(ctx, trailerCase.quote, subject)).toBeNull()
  })
})
...
describe('the three counts satisfy the arithmetic the curation note prints', () => {
  it('pool = offered + narrowedOut + withheld, on every step of a real quote', () => {
    ...
    for (const section of quote.sections) {
      ...
      const offer = stepOffer(ctx, quote, section)
      const why = stepReason(ctx, quote, section)
      if (!why) continue
...
describe('a quote reopened after a reload can still be built', () => {
  it('offers and explains a step whose view id and block id are both gone', () => {
    if (!trailerCase) return
    const live = stepOffer(ctx, trailerCase.quote, trailerCase.section)
    const liveWhy = stepReason(ctx, trailerCase.quote, trailerCase.section)
    ...
    const after = stepOffer(ctx, reopened, section)
    const afterWhy = stepReason(ctx, reopened, section)
    ...
    /* and the two halves the mechanism needs still work: the whole
       ...
       past the narrowing */
    const everything = stepOffer(ctx, reopened, section, { all: true })
  ...
  it('still refuses when the root table itself has gone off the sheet', () => {
    ...
    expect(stepOffer(ctx, orphan, trailerCase.section).candidates).toEqual([])
    expect(stepReason(ctx, orphan, trailerCase.section)).toBeNull()
  })
})
...
describe('what a choice would do to the total, before it is taken', () => {
  it('previews an add, and the round trip back off lands on the number it started at', () => {
    ...
    const priced = stepOffer(ctx, quote, section, { all: true }).candidates.find(
  ...
  it('counts the ADJUSTMENTS a person typed, not just the package', () => {
    ...
    const withTrade: QuoteDef = {
      ...quote,
      adjustments: [{ id: 'adj-1', kind: 'tradeIn' as const, label: 'Trade-in', amount: -4000 }],
    }
  ...
  it('counts the QUANTITY on the line it is weighing', () => {
    ...
    expect(weighPick(quote, three).delta).toBe((weighPick(quote, priced.line).delta as number) * 3)
  })
  ...
  it('says a blank is a blank — never a delta of nothing', () => {
    ...
    if (!found) return
    const { quote, section } = found
    const any = stepOffer(ctx, quote, section, { all: true }).candidates[0]
    if (!any) return
