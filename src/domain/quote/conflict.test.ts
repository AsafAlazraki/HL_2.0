...
import type { FrozenLevel, QuoteAdjustment, QuoteDef, QuoteLine } from '@/domain/model'
...
function quote(lines: QuoteLine[], adjustments: QuoteAdjustment[] = []): QuoteDef {
  return {
    id: 'q1',
    /* three fields the contract now requires of every document: the
       tenant key, the walk frozen with the sections it draws, and the
       per-document audit. This quote names no organisation, walks one
       chapter — its own subject — and nothing has happened to it. */
    orgId: '',
...
describe('levelConflict — nothing to decide', () => {
  ...
  it('says nothing when every line carries the rung and no figure moves', () => {
    const flat = ln(
      'l-flat',
      'Bilge pump',
      [rung('cash', 'Cash', 500), rung('trade', 'Trade', 500)],
      'cash',
    )
...
describe('levelConflict — what moves and what holds', () => {
  const sheet = () =>
    levelConflict(quote([hull(), motor(), fitted(), unpriced()]), 'trade', 'Trade')
  ...
  it('counts the lines rather than saying "one" for all of them', () => {
    const second = ln(
      'l-two',
      'NSM 4.8m',
      [rung('cash', 'Cash', 4_240), rung('trade', 'Trade', 4_000)],
      'cash',
    )
...
describe('levelConflict — the arithmetic, and the act it previews', () => {
  ...
  it('skips an adjustment that is not a finite number', () => {
    const broken = quote([hull()], [{ id: 'a1', kind: 'line', label: '', amount: Number.NaN }])
