...
/* ============================================================
   ...
   ONE MECHANICAL CHANGE AT INTEGRATION, and it is not an assertion:
   `validateEnvelope` takes the business as its second argument in
   HL_2.0. The old signature had one, because the old app had one
   organisation implied everywhere; the contract now requires `orgId`
   on every persisted record, so the validator is handed the business
   whose file this is and stamps what it reads back. The two calls
   below pass it. Nothing either case asserts is about the tenant.
   ============================================================ */

/** the business reading the file — see the note above */
const ORG = 'northside'

describe('a declared ladder through a round trip', () => {
  it('SURVIVES AN EXPORT AND AN IMPORT', async () => {
    const { validateEnvelope } = await import('@/domain/io/envelope')
    const file = {
    ...
    const out = validateEnvelope(file, ORG)
