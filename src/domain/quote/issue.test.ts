...
/* ============================================================
   PORTED 2026-09-17 FROM `features/quote/issue.test.ts`, WHOLE:
   nineteen cases, every assertion unchanged.

   THE REGISTRY IS THE ONE MECHANICAL CHANGE. The old file drove a
   module-level registry — `registerQuote`, `getQuote`, `issueQuote`,
   `setOverride`, `patchQuote`, `discardDraft` — that lived in
   `features/quote/quotes.ts` and wrote through to Dexie. That file
   split in two on the way across: the arithmetic and the refusals are
   `./totals`, the writes are `./commands` (pure, one command per act,
   each with its inverse), and the registry itself is
   `src/state/quotes.ts`, a zustand store `src/domain` may not import.

   So the six registry calls are re-created at the top of this file
   over the pure command layer, with the old signatures, and EVERY
   TEST BODY BELOW IS THE OLD ONE UNCHANGED. The shim is four lines of
   `Map` and one `apply`; it is not a second implementation of
   anything, and the store's own version of the same promise is
   pressed in `src/state/quotes.test.ts`.

   `issueQuote` still answers `true`/`false` here, because that is
   what these nineteen assertions read. The command it calls answers
   with the BLOCKERS' OWN SENTENCES instead — see `issue()` in
   commands.ts — and `commands.test.ts` pins that richer contract. The
   two do not disagree: false is exactly "there was at least one
   sentence".
   ============================================================ */
...
import {
  ...
  issue,
  setCustomer,
  setOverride as setOverrideCommand,
  type QuoteCommand,
} from './commands'
import { issueBlockers, needsOverrideReason, quoteTotals, unexplainedOverrides } from './totals'

import type { QuoteDef, QuoteLine } from '@/domain/model'

/* ---------------------------------------------------------- */
/* The registry, over the commands                            */
/* ---------------------------------------------------------- */

/** ONE CLOCK FOR THE WHOLE FILE. The old registry stamped `nowIso()`
 *  on every write; nothing here reads a stamp, and a fixed one keeps
 *  the file free of the real clock. */
const NOW = '2026-01-08T00:00:00.000Z'

const book = new Map<string, QuoteDef>()

const registerQuote = (quote: QuoteDef): void => {
  book.set(quote.id, quote)
}

const getQuote = (id: string): QuoteDef | undefined => book.get(id)

const discardDraft = (id: string): void => {
  book.delete(id)
}

/** The old `mutate`: a quote that is gone takes no write, an ISSUED
 *  quote refuses every one of them, and what comes back is the
 *  document the command produced. `apply` is the line that makes both
 *  refusals true now, so this is a lookup and a store and nothing
 *  else. */
function run(id: string, command: QuoteCommand): boolean {
  const current = book.get(id)
  if (!current) return false
  const outcome = apply(current, command, NOW)
  if (!isDone(outcome)) return false
  book.set(id, outcome.next)
  return true
}

const issueQuote = (id: string): boolean => run(id, issue())
...
const setOverride = (
  ...
  run(id, setOverrideCommand(lineId, price, reason))
}

/** The old `patchQuote` took a `Partial<QuoteDef>`; the only key this
 *  file patches is the customer, which is `setCustomer`'s whole job. */
const patchQuote = (id: string, patch: Pick<QuoteDef, 'customer'>): void => {
  run(id, setCustomer(patch.customer))
}
...
function draft(over: Partial<QuoteDef> = {}): QuoteDef {
  ...
  const quote: QuoteDef = {
    id: `q${n}`,
    /* THREE FIELDS THE CONTRACT NOW REQUIRES and the old literal did
       not carry: `orgId` on every persisted record, and `chapters`
       and `events` on every quote. Stricter typing, never a changed
       assertion — nothing below reads any of the three. */
    orgId: 'northside',
    reference: `Q-${n}`,
    state: 'draft',
    viewId: 'view_1',
