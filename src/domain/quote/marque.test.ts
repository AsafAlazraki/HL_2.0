...
/* STRAIGHT AT THE SPLITTER. The old suite reached it through
   `QuoteBuild.tsx`, which re-exported it (`export { marqueOf } from
   './marque'`) so the screen and the test named one function; the
   screen does not come across and the function does. Same function,
   one import shorter. */
