...
**What this round closed**, against the second critic's `docs/directions/built-critique-m2-close.md`: both blockers (every shut model is named; a customer never becomes something Northside sells); the picker rebuilt as a showroom of makers and boats with their photographs; the sale's screens and now Entry speak the dealer's words; the app is the day, blue and white, with the night offered; a quoted person is a customer at once; Ctrl K sells a boat, reads codes and answers "trailer for sp560"; the Pictures door is pictures first; the spine says every fact whole; Home's photograph is the build's; Home's search acts on Enter; the register, History and a customer's page draw their one record large. The verifier's cold drive found four faults the gate had not, and fixed and pinned each: the price list scrolled sideways by 15 px at 834–1440; Home's search did nothing on Enter; Entry still spoke the image ledger ("Held photograph … in the image ledger since …") and "rows"; Data's spread called the sheet one grid in the file's order. Per screen and per size: `docs/directions/built-m2-close-2.md`. Every screen was re-photographed at 1440, 1280, 1920 and 390; no `FAULT-*` shot exists.

**Still wrong, ranked:** Home counts 810 "boats" that are lines, beside the picker's 289 models; engine words on the register's panel ("frozen lines"), History's teaching state ("register", "rung") and the paper's side panel, and "53 tables · 15,691 rows" on the register's and History's heads; the issued build's caption still offers "02 Motor"; at 390 the ADV7's act is below the fold; some wide floor (the sheet's spine beside an open record at 1920, the register's band tiles); Home's makers' marks are not doors; the sheet's cursor after a chapter change and its terse "Cell edit" step line; `/favicon.ico` 404; WCAG 2.1.4 owed. **The owner has looked at none of it** — every screen is PROVISIONAL.

## MILESTONE 2: THE FIX ROUND IS CLOSED (2026-09-24)

**The gate, alone, on this tree:** `npm test` — 193 test files, 3,276 tests, 14 static rules, no failures (203 s on an idle desk); `npm run build` — green, about 4 s, no warning; `npm run e2e` — 1,188 tests, 910 passed, 278 skipped by design, 0 failed, 53.7 minutes with 2 workers, run alone.

**What this round closed.** Every blocker and major in `docs/directions/built-critique-m2.md` but one is closed on the built app, driven cold: a found row opens THAT row with its record (from Home, from inside the sheet, and on a reload); page 1 of the customer's quotation reads NORTHSIDE MARINE; the ADV7's colourways stand above the act; History's day reads `started · addressed · issued` at every size; the sheet is redrawn as the maker's price list and holds 20 of 18 at 1280 × 800 (Data 18, Customers 22, History 21, Quotes 19); the pill's Quotes count agrees with the register; every desk screen fits 800, 900 and 1,080; no amber act refuses at rest; a write keeps the sheet's folds; no router pattern or plan word reaches a dealer; no keycap on a phone. The one major left open is #17, empty floor at wide sizes where one record is the honest state — better, not closed.

**What the close pass itself found by driving, and fixed** (the gate was green over all of it): a row found from inside the sheet, or its `?at=` reloaded, did not open (a virtualised list's estimate, and the router's scroll restoration writing a pixel offset back into it); "Shut" on the found row's model did nothing; the spine's figure per material was cut at 1280 and a render's caption fell under its block at 1920; the lit row's keys were clipped at 1280; the finder could not find "SP560 B-B-B" or "Highfield SP560"; Data dated the customers register in UTC; the dead end said the business had no name while the file was being read; the last "Milestone 6" on a dealer's screen; `Other Charges` printed `0` beside `$0`; "1 models". Each is a dated line in `docs/DECISIONS.md` and each is pinned by a test that fails on the old tree. The first `npm test` of the pass was red on one stale route test of the dead end's doors; it pins the same property now.

**Still wrong, ranked:** the finder does not read the dealer's own codes (`HBS126` finds nothing in Ctrl K; the sheet's own field does); image-ledger language ("Held copy 1,100 × 619, never enlarged") on Entry, the picker, the build and the cascade; the register's empty state is still a wireframe diagram and scrolls inside its panel at 1280; empty floor at 1920 on History, Customers and a register whose boat has no hero; Data's pairing order depends on how the sheet was loaded; the sheet's tablet placeholder is cut and its shut line can jump above the window; a 404 for `/favicon.ico` on every load; WCAG 2.1.4 owed. Full list with measurements, per screen and per size: `docs/directions/built-m2-close.md`. Every shot under `docs/directions/<screen>/built/` was re-taken on this tree; the three `FAULT-*` shots are deleted because their faults are fixed.

**The owner has looked at none of it.** Every screen is still PROVISIONAL in `docs/SCREENS.md`.
