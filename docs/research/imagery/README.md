...
- **Five Stacer overhead frames are stored landscape and hang portrait.** They carry EXIF orientation 6, so the file header says 1776 × 1180 and this ledger records 1180 × 1776 — what a browser shows. They were the only pixel sizes in the whole ledger that moved when it was re-measured, and a size read straight off the header has those two the wrong way round.
- **A 429 is a fact about the sweep, not about the address.** A full re-run on 2026-09-17 asked `www.formosamarineboats.com.au` for 218 files in a few minutes and 165 of them came back 429 after answering 200 minutes earlier; every one served again when asked one at a time with a 2 s gap. The tool now refuses to write a 429 over a status already recorded, waits after the first two, and leaves a host alone once it has answered 429 three times.
- **Byte sizes drift downward on the two hosts that re-optimise their library.** Of 500 addresses re-measured on 2026-09-17, 13 came back 3–13 % smaller than the same address measured hours earlier: 12 on `www.formosamarineboats.com.au`, one on `www.stacer.com.au`. No pixel size moved on any of them, and nothing else in the 500 disagreed.
